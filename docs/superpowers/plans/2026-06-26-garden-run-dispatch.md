...
> **Decision note:** Only hoist if it stays a clean win. The asymmetry is real — `registry` is shared by all three but `projection`/`now`/`signal` are partial. The bag must contain ONLY the dispatch+route plumbing; eligibility/bookkeeping inputs (`registry`, `projection`, `maxJobs`, `question`) stay as the `& {…}` extras. If, when you get here, the intersection types fight `exactOptionalPropertyTypes` badly enough to obscure the win, STOP and keep Tasks 2–4 (the bag built inside each runner) — that already kills the worst lockstep (the two-spread duplication inside each runner). Record the stop reason in the commit message. This is the generalize-only-if-cleaner gate.
