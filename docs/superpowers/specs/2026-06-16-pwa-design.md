...
**Deferred to v1.1+ (YAGNI):** voice-for-*ask* + TTS, push nudges, a page viewer / recents deep-links, the Capacitor native wrapper, multi-user/accounts, phone-side page authoring (stays a desktop activity per the client model), per-device tokens, **service worker** (the SW registration + offline app-shell caching are v1.1; the manifest ships in v1 served from `/assets/` so add-to-home-screen works, but SW is not registered), **offline capture-queue wiring** (`CaptureQueue` is built and tested but not yet wired — v1.1).

## Architecture

- **Stack:** React + Vite, built to static assets. **Served by `dome ask-server`** as a static-file surface: `GET /` returns the app shell (`index.html`), `GET /assets/*` the hashed bundles, with an SPA fallback to `index.html` for client routes. One origin ⇒ **no CORS**; the app fetches only the server it was served from. (This relocates the server's current `GET /` JSON ping to an API path, e.g. `GET /healthz`.)
- **One new backend route — `POST /transcribe`:** accepts an audio blob (multipart or raw body), shells out to a **local whisper** on the host (whisper.cpp / faster-whisper), returns `{ text }`. Same subprocess discipline as the model provider (a host-configured command; audio never leaves the host). Runtime-free — subprocess only, no vault open; bounded body + bearer auth. Runs authenticated but outside the vault mutex (so a slow whisper subprocess cannot block `/ask`, `/tasks`, etc.), with a per-request subprocess timeout. (Note: `/capture` also touches no vault at runtime but does run under the mutex — `/transcribe` does not.) **The two server changes for v1 are this route and the static-serving above**; all data routes already exist.
- **Auth (the carve-out that makes a browser app work):** the data/API routes stay bearer-gated, but a browser's initial navigation to `GET /` + `/assets/*` cannot carry an `Authorization` header — so the **app shell and static assets are served unauthenticated**. That is safe here: the shell holds no secrets, the endpoint is a private Tailscale/loopback trust domain, and every route that touches vault data still requires the token. `tokenGate` prompts for the token on first run, stores it in `localStorage`, and attaches it to all API calls. The ask-server's `fetch` therefore branches: static GETs (shell/assets) → served directly (no auth, no mutex); everything else → auth-before-routing exactly as today. (Single shared token; per-device tokens deferred.)
- **Offline / PWA:** web manifest served from `/assets/` (hashed, via Vite `?url` import in `main.tsx`) — add-to-home-screen works in v1. **Service worker and offline capture-queue wiring are deferred to v1.1**: the SW registration and offline app-shell caching are not present in the v1 build; `CaptureQueue` is built and tested but not yet wired to the Composer. A capture created while offline in v1.1 will be queued in IndexedDB and flushed on next app-foreground (iOS has no background sync — server stays canonical).
