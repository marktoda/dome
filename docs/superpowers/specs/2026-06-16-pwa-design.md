...
- **One new backend route — `POST /transcribe`:** accepts an audio blob (multipart or raw body), shells out to a **local whisper** on the host (whisper.cpp / faster-whisper), returns `{ text }`. Same subprocess discipline as the model provider (a host-configured command; audio never leaves the host). Runtime-free — subprocess only, no vault open, no mutex (like `/capture`); bounded body + bearer auth. **The two server changes for v1 are this route and the static-serving above**; all data routes already exist.
