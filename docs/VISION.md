...
- **A CLI for explicit operations.** `dome serve` and `dome sync` drive the compiler host; `dome status`, `dome inspect`, `dome doctor`, and `dome answer` expose recovery; `dome query`, `dome lint`, `dome export-context`, `dome today`, and `dome prep` read from adopted state; `dome rebuild` rebuilds the projection store from markdown. Invokable from any shell.
...
**Tend the garden periodically.** Weekly or monthly, `dome lint` reports stale claims, orphan pages, missing cross-references, contradictions, and schema violations from adopted state. When the engine needs a human decision, it asks a question; the user answers through `dome answer`; recovery mutations stay inside normal processor/effect/capability routing instead of one-off admin commands.
