...
- **A CLI for explicit operations.** `dome serve` and `dome sync` drive the compiler host; `dome status`, `dome check`, and `dome resolve` are the normal see-and-fix path; `dome query` and `dome export-context` read from adopted state for recall, planning, and handoff context; `dome inspect`, `dome doctor`, `dome lint`, `dome answer`, `dome rebuild`, and hidden compatibility views remain available for debugging, compatibility, and maintenance. Invokable from any shell.
