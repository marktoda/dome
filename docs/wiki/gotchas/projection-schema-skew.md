...
- The rebuild is logged in the run ledger (`status: "succeeded"`, `processor_id: "engine.projection-rebuild"`). Reviewable via `dome doctor --show runs --processor engine.projection-rebuild`.
- For large vaults (50k+ pages), the rebuild may take a minute or two. `dome doctor --show rebuild-progress` provides a tail-of-walk view.
