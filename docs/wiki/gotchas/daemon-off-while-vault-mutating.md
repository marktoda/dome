...
- **Use `dome doctor --time-since-reconcile`** (preserves the v0.5 flag name for compatibility; reads the `.dome/state/last-reconcile-mtime.txt` marker) to surface drift age in CI / scripts / habitual checks. The metric tells the user when to schedule a sync if they've been running ad-hoc.
