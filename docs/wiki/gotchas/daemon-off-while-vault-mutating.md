...
# Compiler host off while vault mutating

**Symptom:** no compiler host is running. The user continues writing into the vault — via Claude Code, vim, Obsidian, or any other consumer shell. Hours or days later, they restart `dome serve` (or invoke `dome sync` directly) and the catch-up phase takes noticeably longer than usual: the host's catch-up scans, the engine constructs a Proposal spanning the accumulated `refs/dome/adopted/<branch>..HEAD` plus working-tree diff (per [[wiki/specs/adoption]]), adoption-phase processors run, garden-phase processors fire on the post-adopt state (intake processors compile every accumulated `inbox/<bucket>/` file, scheduled processors fire for every cron interval that elapsed). Sometimes the catch-up triggers many LLM-backed runs (every accumulated `inbox/raw/` file invokes `dome.agent.ingest`), accumulating real API cost.

**Root cause:** Dome's compiler is host-shaped. `dome serve` is the first host and may run foreground like an LSP/watch process or under a background service. When a host is running, it constructs a Proposal within sub-second latency of each committed native write; the engine adopts; garden processors fire shortly after. When no host is active, native writes still happen — Claude Code's `Write`, vim, Obsidian — but no listener is observing them in real time. The changes queue up *implicitly* as git working-tree state and `inbox/<bucket>/` filesystem state; `dome sync` drains the queue by constructing a single Proposal that captures all accumulated changes and routing it through the adoption loop, then garden processors fire on the post-adopt state.
...
- **LLM cost (when intake processors fire)** — proportional to the number of accumulated captures and source-backed synthesis runs they trigger. Each raw file triggers the corresponding garden-LLM processor (`dome.agent.ingest` for `inbox/raw/*`; analogues for opt-in buckets). *This is the only cost component that's noticeable in dollars and time.* The `model.invoke` capability's `maxDailyCostUsd` cap (per [[wiki/specs/capabilities]] §"model.invoke") backstops the spend; beyond the cap, the processor's `modelInvoke` calls are denied and the work is deferred to the next day's sync.
...
- Don't disable garden-LLM processors to "avoid the catch-up cost." If you don't want LLM-driven ingest at all, disable the `dome.agent` bundle in `<vault>/.dome/config.yaml`. Disabling on the fly to avoid catch-up just defers the cost until you re-enable.
...
**Counter-example (the bad case before mitigation):** A user leaves the compiler host off for two weeks while traveling. The vault accumulates 200 inbox/raw/ captures and dozens of native wiki edits. On return, the user runs `dome serve`; the catch-up at startup constructs a Proposal spanning the full range; adoption runs; the post-adopt state fires many garden-LLM intake and synthesis runs back-to-back. The first budgeted tranche of LLM spend happens; the cap denies further model invokes; the remaining captures sit until the next day's sync. With the operational mitigations: a foreground `dome serve` during active sessions or a launchd service throughout the trip would have kept the adopted ref advancing daily (each capture compiled within minutes of being written). The trip's worth of compilation cost was the same total; it just spread over time and didn't compress into one cap-exhausting burst.
