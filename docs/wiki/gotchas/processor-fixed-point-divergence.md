...
The diagnostic surfaces via `dome submit` exit code, `dome doctor --show recent-processor-divergence`, and the `engine.adoption.blocked` event.
...
dome submit: blocked main: proposal prop_1748... (1 diagnostic)
  - [block] fixed-point.divergence: adoption loop hit MAX_ITER=100 without convergence.
    ...
    Recommended actions:
      ...
      2. Inspect the recent diagnostics via `dome doctor --show recent-processor-divergence`.
