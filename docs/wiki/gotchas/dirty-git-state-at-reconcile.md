...
- For users with long-running merges: `dome status` reports the dirty state at startup; the engine-emitted `dome.git.dirty-state` DiagnosticEffect (visible via `dome show diagnostics`) carries the resolution hint.
