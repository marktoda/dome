...
The deferred `dome doctor --check-substrate-counts` mode parses spec prose for inlined counts that cite a canonical surface (e.g., "12 invariants per `INVARIANTS`") and compares against the actual const. v0.5 ships the convention but not the linter; v0.5.1+ may add the check if substrate growth makes manual link-checking expensive.
