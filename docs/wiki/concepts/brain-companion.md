...
- **Streamlined to talk to.** Quick-capture is intended to be a file write to `inbox/raw/`: no app to launch, no form to fill. The `dome.intake` garden processors now compile raw captures into generated pages and route low-confidence extracted items through `dome answer`; stale-inbox diagnostics and richer synthesis remain roadmap work. See [[wiki/specs/processors]] §"First-party processors" and [[wiki/matrices/intent-prompt-processors]].
