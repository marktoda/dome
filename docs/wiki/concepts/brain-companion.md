...
- **Streamlined to talk to.** Quick-capture is intended to be a file write to `inbox/raw/`: no app to launch, no form to fill. The `dome.intake` garden processor that compiles captured content is still roadmap work; deterministic daily creation, carry-forward, search, and recovery are the shipped v1 substrate today. See [[wiki/specs/processors]] §"First-party processors" and [[wiki/matrices/intent-prompt-processors]].
