...
3. **Failed, timed-out, and cancelled processor runs still complete the ledger row.** The executor boundary converts thrown processors, invalid output, timeouts, and cancellations into structured terminal results. The runtime persists those results as `status: 'failed'`, `status: 'timed_out'`, or `status: 'cancelled'` with structured `processor.threw`, `processor.invalid-output`, `processor.timeout`, or `processor.cancelled` errors. These outcomes are durably observable.
