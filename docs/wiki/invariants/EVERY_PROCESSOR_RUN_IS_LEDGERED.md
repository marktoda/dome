...
1. **The engine begins a RunRecord on every processor dispatch.** `src/processors/runtime.ts`'s direct runner path calls `insertQueued` (via `src/ledger/runs.ts`) before invoking `processor.run()`, transitions through `markRunning`, and currently writes `markSucceeded` / `markFailed`. The ledger terminal write surface is `markSucceeded` / `markFailed` / `markTimedOut` / `markCancelled`; the executor-integrated timeout and cancellation paths use the new terminal accessors after Task 4.
2. **The applier writes `capability_uses` rows per effect.** Joined to the RunRecord by `run_id`.
3. **Failed, timed-out, and cancelled processor runs still complete the ledger row.** A `try/catch` around `processor.run()` writes `status: 'failed'`, `error: <message>`, before rethrowing. The timeout and cancellation terminal accessors write `status: 'timed_out'` / `status: 'cancelled'` with structured `processor.timeout` / `processor.cancelled` errors once the executor path is wired through the runtime. These outcomes are durably observable.
