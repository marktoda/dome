...
Pre-Phase-B, the wrap was duplicated across `bindTools` (in `registry.ts`) and `bindAiSdkTools` (in `ai-sdk-binding.ts`). The duplication was caught by the architecture review and closed structurally by extracting `wrapMutatingInvoke` to a single source. The axiom is the substrate-shape pin that prevents the duplication from reappearing: a future contributor adding a third projection (`renderHttp`, `renderVoice`, or similar protocol renderer) consumes the helper rather than inlining the dispatch loop; a future change to the wrap (causation metadata, backpressure gate, a `closed`-flag pre-check coordinating with the post-close silent-no-op behavior described in [[wiki/specs/sdk-surface]] §"Vault lifecycle") is made in one place and inherited by every projection.
...
- **The single-source helper.** `wrapMutatingInvoke(vault, entry, writer)` in `src/tools/registry.ts` is the only function in the codebase that constructs the post-invoke `dispatchEvents` call. `bindTools` (the source for `vault.tools`) consumes it; `bindAiSdkTools` (the source for `projectAiSdk(vault)`) consumes it; protocol renderers (`renderMcp` and future siblings) consume `surface.tools` rather than re-binding (so they inherit the wrap by construction). The two integration tests below catch behavioral divergence — a wrap that drifts in behavior fails them; the v0.5.1+ semantic linter at [[wiki/linters/wrap-mutating-invoke-consumption]] closes the gap for byte-equivalent hand-inlined duplicates (a duplicate that happens to be correct until `wrapMutatingInvoke` next changes, which the integration tests cannot catch).
