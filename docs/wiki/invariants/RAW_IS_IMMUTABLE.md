...
**Test guarantee:** `tests/invariants/raw-is-immutable.test.ts` — two cases. (1) A fixture processor emits a PatchEffect against `raw/example.md`; asserts the broker denies the effect with `code: "capability-deny-patch"`. (2) A fixture vault has a committed modification to `raw/example.md`; `dome sync` returns `adopted: false` with a `raw.immutable` blocking diagnostic naming the path.
