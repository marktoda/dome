...
3. **`log.md` is a projection of the run ledger** maintained by `dome.log`'s adoption-phase processor. The user-facing `log.md` view is reconstructable from `runs.db` + `outbox.db` via `dome doctor --repair`.
