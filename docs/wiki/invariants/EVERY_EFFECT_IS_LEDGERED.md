...
3. **`log.md` is a projection of the run ledger** maintained by `dome.log`'s adoption-phase processor. The user-facing `log.md` view is reconstructable from `runs.db` + `outbox.db` via the `dome.log:rebuild` processor — invoked today through `dome run-processor dome.log:rebuild` and in v1.x as part of the reserved `dome doctor --repair` flow.
