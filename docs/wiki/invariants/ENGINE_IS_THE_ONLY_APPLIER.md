...
1. **Exhaustive routing.** `src/engine/apply-effect.ts` carries a `switch (effect.kind)` over the ten Effect kinds for generic sink routes, and `src/engine/garden-patch-dispatch.ts` owns the garden PatchEffect special case. TypeScript's `never`-type exhaustiveness check fires on any unrouted generic kind. Adding another Effect without updating the route layer fails compilation.
