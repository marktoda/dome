...
**The explicit "derived" zone under `.dome/`:** Three subdirectories — `.dome/in-flight/`, `.dome/state/`, `.dome/cache/` — hold derived operational state, not canonical knowledge. They are gitignored by default and rebuildable from canonical state. Deleting any of them does not lose user content; it just causes the next reconciliation pass to do more work. The `.dome/config.yaml`, `.dome/page-types.yaml`, `.dome/prompts/`, `.dome/hooks/`, `.dome/tools/`, and `.dome/cli/` paths are part of the vault's identity and ARE committed. The distinction is documented in [[wiki/specs/vault-layout]] §"Derived operational state under `.dome/`".
