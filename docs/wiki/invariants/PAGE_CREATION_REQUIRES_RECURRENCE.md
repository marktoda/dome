...
**Structural enforcement:** When enabled, `writeDocument` requires `opts.reason` whenever `opts.create` is `true` (see [[wiki/specs/sdk-surface]] §"Tool signatures" for the canonical input shape). Missing `reason` → `Result.err({ kind: 'page-creation-requires-reason' })`. The reason is logged with the page-creation log entry. When disabled, `opts.reason` is optional and unenforced.
