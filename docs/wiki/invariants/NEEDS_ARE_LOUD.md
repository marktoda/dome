...
4. **Older vaults retained stale grants.** A vault that predated a bundle's newer behavior kept its old grant lists and silently lost that behavior—the kind was granted but the specific entry was not. Doctor now names explicit repairs; narrow init does not mutate owner config.
