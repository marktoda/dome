...
- **Reconciliation** uses `git diff --name-only refs/dome/adopted/<branch> HEAD` + `git status --porcelain` to detect what changed since the last `dome sync` (per [[wiki/invariants/ADOPTED_REF_IS_SEMANTIC_CURSOR]] — the adopted ref is the canonical cursor). No separate hash-cache code path is needed.
...
- **Multi-device sync (v1+)** is `git push` / `git pull` against a remote, then `dome sync` to fire hooks against the synced changes and advance the adopted ref.
...
- [[wiki/specs/adoption]] — the state machine `dome sync` drives
