...
"Eventually" here means: at most by the time `dome reconcile` finishes — either when fired by the watcher in a running `dome serve` (sub-second latency under normal load), or when `dome reconcile` is invoked manually after the daemon was off.
...
2. **Reconcile-driven (secondary, fills the daemon-off gap).** `dome reconcile` runs three phases per [[wiki/specs/cli]] §"`dome reconcile`": inbox processing, git-diff replay (fires `document.written.<category>.<type>` for every file changed since `.dome/state/last-reconciled-sha.txt`), scheduled-hook catchup. The git-diff replay is idempotent and durable — events the watcher missed (because the daemon was off, or because the OS coalesced chokidar events under load) get fired during reconcile and processed by the same hook chain.

The two-layer design means: while `dome serve` is running, the watcher catches writes promptly. While it's off, drift accumulates as a queue of git-tracked changes — picked up by `dome reconcile` at next startup, with cost growing linearly with time-since-last-reconcile (see [[wiki/gotchas/daemon-off-while-vault-mutating]]). The vault converges on consistency either way.
...
**Test guarantee:** A regression test (proposed alongside the watcher-driven reactive-hook implementation) writes a file directly to the vault filesystem (bypassing Tools), waits for `vault.out-of-band-edit` to fire and be processed, asserts that `index.md` reflects the new content and `log.md` carries a `source: 'out-of-band'` entry. A second test sets aside the daemon, makes a similar write, runs `dome reconcile`, asserts equivalent end-state.
