...
2. **Recoverability.** A terminally-failed external action is visible via `dome doctor --show outbox`; the user can replay or abandon via `dome doctor --outbox-replay <key>` / `--outbox-abandon <key>`.
