...
2. **Privileged API is module-private.** The dispatcher's `writeIndex` and `appendLogEntry` are not exported from the SDK's public surface. The `appendLog` Tool is implemented as a thin wrapper that calls `dispatcher.appendLogEntry` internally. Shipped-default hooks (`auto-update-index`) reach the privileged API through a `dispatcher` field on the `HookContext` exposed only to built-in handlers — plugin and vault-local hooks receive a `HookContext` without that field.
