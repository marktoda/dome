...
2. **The manifest-level rule makes the unsafe combination impossible by construction.** No garden `model.invoke` processor declares `graph.write`. With that rule held, the only durable effects a garden model processor can emit are `QuestionEffect` (durability supplied by `answers.db`) and a narrowly-scoped `PatchEffect` (durability supplied by the committed markdown that is the `PatchEffect`'s output) — both rebuild-safe. The shipped model agents follow this: `dome.agent.consolidate` declares `read` + `model.invoke` + `patch.auto` + `question.ask` and never `graph.write`. Its integrity-review findings (emitted via the `flagIntegrity` tool, folded in from the retired `dome.warden.integrity`) surface as `DiagnosticEffect`s — transient model judgment that regenerates each nightly run and self-clears when the page is reconciled, so nothing durable is lost on rebuild; its durable output is committed markdown via `PatchEffect`. `dome.agent.ingest` follows the same rule.
