...
2. **The manifest-level rule makes the unsafe combination impossible by construction.** No garden `model.invoke` processor declares `graph.write`. The shipped semantic gardener declares `read` + `model.invoke` + `patch.propose` + `proposals.read`; its knowledge-integrity findings are transient `DiagnosticEffect`s and its durable knowledge output exists only after an owner adopts proposed markdown. Ingest and brief follow the same no-facts rule for their narrower automatic patches.
