...
| "Review my task backlog" | shipped product read + mutation seam | `dome.daily.task-backlog` + `performSettleBatch` | view (`GET /task-backlog`) + workspace mutation (`POST /task-backlog/review`) | none | ViewEffect (`dome.daily.task-backlog.list/v1`) then one locked human commit (`dome.task-backlog.review/v1`) |
