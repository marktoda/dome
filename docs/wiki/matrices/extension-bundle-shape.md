...
|---|---|---|---|---|---|---|
| **`dome.markdown`** *(first-party)* | `shipped` | — | — | `validate-wikilinks.ts`, `ambiguous-wikilink-answer.ts`, `repair-wikilinks.ts`, `simplify-indexes.ts`, `normalize-frontmatter.ts`, `lint-frontmatter.ts`, `page-status.ts`, `lint-supersession.ts`, `broken-images.ts`, `duplicate-detection.ts`, `duplicate-detection-answer.ts`, `stale-dates.ts`, `refresh-updated.ts`, `raw-immutable.ts`, `core-size.ts`, `orphan-pages.ts` | — | `read: ["**/*.md", ".dome/page-types.yaml", images, "raw/**"]`; `patch.auto: ["**/*.md", "wiki/syntheses/*.md", "wiki/**/index.md"]`; `graph.write: ["dome.page.*"]` (frontmatter status/supersession facts from `page-status.ts`); `question.ask: true` |
...
| **`dome.agent`** *(first-party)* | `shipped` | — | —; future: `preamble.md` | `ingest.ts`, `inbox-stale-check.ts`, `consolidate.ts`, `brief.ts`, `preference-signals.ts`, `preference-promotion.ts`, `preference-promotion-answer.ts` | — | `read: ["wiki/**/*.md", "notes/**/*.md", "inbox/**/*.md", "index.md", "log.md", "consolidation-ledger.md", "sources/calendar/*.md", "core.md", "preferences/signals.md"]`; `patch.auto: ["wiki/**/*.md", "notes/**/*.md", "index.md", "log.md", "inbox/processed/*.md", "inbox/raw/*.md", "consolidation-ledger.md", "preferences/signals.md"]` — `core.md` is read-only in the bundle grant: the propose-only core-memory page appears in exactly ONE `patch.auto` declaration, `preference-promotion-answer`'s, which receives a narrow per-processor replacement grant (`read` + `patch.auto` over `core.md` + `preferences/signals.md` only) per [[wiki/specs/preferences]] §"The single-auto-writer exception" (the `brief` processor declares only `wiki/dailies/*.md` + `notes/*.md` + `preferences/signals.md` for `patch.auto`, but its `read` declaration includes `inbox/**/*.md` + `consolidation-ledger.md` so the scoped questions batch keeps ingest- and consolidate-raised questions); `model.invoke: { maxDailyCostUsd: 10 }` on the three agents; `question.ask: true` for owner clarifications and promotion questions; `graph.write: ["dome.preference.*"]` for the deterministic `preference-signals` counter ONLY — the model processors still declare no `graph.write` (MODEL_PROCESSORS_EMIT_NO_DURABLE_FACTS) |
