...
| **`dome.agent`** *(first-party)* | `shipped` | — | —; future: `preamble.md` | `ingest.ts`, `inbox-stale-check.ts`, `consolidate.ts`, `brief.ts` | — | `read: ["wiki/**/*.md", "notes/**/*.md", "inbox/**/*.md", "index.md", "log.md", "consolidation-ledger.md", "sources/calendar/*.md", "core.md"]`; `patch.auto: ["wiki/**/*.md", "notes/**/*.md", "index.md", "log.md", "inbox/processed/*.md", "inbox/raw/*.md", "consolidation-ledger.md"]` — `core.md` is read-only by design: the propose-only core-memory page appears in no `patch.auto` declaration ([[wiki/specs/autonomous-agents]] §"Core-memory injection") (the `brief` processor declares only the daily-note targets `wiki/dailies/*.md` + `notes/*.md` for `patch.auto`, but its `read` declaration includes `inbox/**/*.md` + `consolidation-ledger.md` so the scoped questions batch keeps ingest- and consolidate-raised questions); `model.invoke: { maxDailyCostUsd: 10 }`; `question.ask: true` for owner clarifications during ingest; NO `graph.write` |
