...
| **`dome.daily`** *(first-party)* | `partially shipped` | `daily`; future: `weekly` | — | `create-daily.ts`, `task-index.ts`, `carry-forward.ts`; future: `create-weekly.ts`, `agenda-with.ts`, `week-review.ts`, `prep.ts`, `append-followup.ts` | — | `read: ["wiki/**/*.md"]`; `patch.auto: ["wiki/dailies/*.md"]`; `graph.write: ["dome.daily.*"]`; `question.ask: true` |
