...
| **`dome.daily`** *(first-party)* | `shipped` | `daily`; future: `weekly` | — | `agenda-with.ts`, `ambiguous-followup-answer.ts`, `carry-forward.ts`, `close-scaffold.ts`, `compose-blocks.ts`, `create-daily.ts`, `normalize-task-syntax.ts`, `prep.ts`, `reconcile-tasks.ts`, `stamp-block-id.ts`, `task-backlog.ts`, `task-index.ts`, `today.ts`; future: `create-weekly.ts`, `week-review.ts`, `append-followup.ts` | — | Bundle grant: `read: ["wiki/**/*.md", "notes/*.md", "sources/calendar/*.md", "sources/slack/*.md"]`; `patch.auto: ["wiki/**/*.md", "notes/*.md"]`; `graph.write: ["dome.daily.*"]`; `question.ask: true`; `questions.read: true`; `proposals.read: true`. `dome.daily.today` has a per-processor replacement grant with `read: ["**"]`, `questions.read: true`, and `proposals.read: true` because the canonical owner-attention view retains question/proposal SourceRefs from every producer; the broad provenance read does not widen other daily processors. `dome.daily.task-backlog` needs only the daily bundle's readable Markdown paths and projection reads; it has no mutation or owner-attention capability. |
