...
| **`dome.intake`** *(first-party)* | `partially shipped` | `capture` | —; future: `preamble.md` | `capture-index.ts`, `extract-capture.ts`, `inbox-stale-check.ts`, `low-confidence-answer.ts`, `synthesize-capture.ts` | — | `read: ["inbox/**/*.md", "wiki/generated/intake/*.md"]`; `patch.auto: ["wiki/generated/intake/*.md", "wiki/syntheses/*.md", "inbox/processed/*.md", "inbox/raw/*.md"]`; `graph.write: ["dome.intake.*"]`; `model.invoke: { maxDailyCostUsd: 5 }`; `question.ask: true`; future: follow-on jobs |
