...
- **Preamble fragment** — `<bundle>/preamble.md` contributes a markdown subsection to AGENTS.md's templated `## Extension conventions` section, regenerated by `dome doctor --repair`. The fragment teaches the agent the bundle's conventions (page-type shape, processor invocations, capability grants).
