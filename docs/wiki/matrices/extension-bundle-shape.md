...
| **`dome.agent`** *(first-party)* | `shipped` | — | —; future: `preamble.md` | `ingest.ts`, `inbox-stale-check.ts`, `consolidate.ts`, `sweep.ts`, `sweep-answer.ts`, `brief.ts`, `preference-signals.ts`, `preference-promotion.ts`, `preference-promotion-answer.ts` | — | `read: ["wiki/**/*.md", "notes/**/*.md", "inbox/**/*.md", "index.md", "log.md", "consolidation-ledger.md", "sweep-ledger.md", "sources/calendar/*.md", "core.md", "preferences/signals.md"]`; `patch.auto: ["wiki/**/*.md", "notes/**/*.md", "inbox/processed/*.md", "inbox/raw/*.md", "consolidation-ledger.md", "sweep-ledger.md", "preferences/signals.md"]` — `index.md` and `log.md` are read-only (same grant shape as `core.md`): the index is a generated render of `description:` frontmatter and `log.md` is frozen history per [[wiki/invariants/NO_ACCRETING_REGISTRIES]], with the grant-aware tools denying stray writes at tool time; `core.md` is read-only in the bundle grant: the propose-only core-memory page appears in exactly ONE `patch.auto` declaration, `preference-promotion-answer`'s, which receives a narrow per-processor replacement grant (`read` + `patch.auto` over `core.md` + `preferences/signals.md` only) per [[wiki/specs/preferences]] §"The single-auto-writer exception" (the `brief` processor declares only `wiki/dailies/*.md` + `notes/*.md` + `preferences/signals.md` for `patch.auto`, but its `read` declaration includes `inbox/**/*.md` + `consolidation-ledger.md` so the scoped questions batch keeps ingest- and consolidate-raised questions; the `sweep` processor reads `sweep-ledger.md` as the advisory cursor file and patches `wiki/entities/**/*.md` + `wiki/concepts/**/*.md` + `sweep-ledger.md` via a narrower per-processor patch.auto); `model.invoke: { maxDailyCostUsd: 5 }` on ingest and brief (each declared cap bounds that processor's OWN daily spend; the extension-wide pool is the vault grant's job — [[wiki/specs/capabilities]] §"model.invoke"); `model.invoke: { maxDailyCostUsd: 10 }` on consolidate and sweep; `question.ask: true` for owner clarifications and promotion questions; `graph.write: ["dome.preference.*"]` for the deterministic `preference-signals` counter ONLY — the model processors still declare no `graph.write` (MODEL_PROCESSORS_EMIT_NO_DURABLE_FACTS) |
