...
|---|---|---|---|---|---|---|
| **`dome.markdown`** *(first-party)* | `shipped` | — | — | `validate-wikilinks.ts`, `ambiguous-wikilink-answer.ts`, `repair-wikilinks.ts`, `normalize-frontmatter.ts`, `lint-frontmatter.ts`, `broken-images.ts`, `duplicate-detection.ts`, `duplicate-detection-answer.ts`, `stale-dates.ts`, `refresh-updated.ts`, `raw-immutable.ts`, `orphan-pages.ts` | — | `read: ["**/*.md", ".dome/page-types.yaml", images, "raw/**"]`; `patch.auto: ["**/*.md", "wiki/syntheses/*.md"]`; `question.ask: true` |
...
| **`dome.intake`** *(first-party)* | `shipped` | `capture` | —; future: `preamble.md` | `capture-index.ts`, `extract-capture.ts`, `inbox-stale-check.ts`, `low-confidence-answer.ts`, `synthesize-capture.ts`, `synthesize-rollup.ts` | — | `read: ["inbox/**/*.md", "wiki/generated/intake/*.md", "wiki/syntheses/intake-*.md"]`; `patch.auto: ["wiki/generated/intake/*.md", "wiki/syntheses/intake-*.md", "inbox/processed/*.md", "inbox/raw/*.md"]`; `graph.write: ["dome.intake.*"]`; `model.invoke: { maxDailyCostUsd: 5 }`; `question.ask: true` for extract + capture-index low-confidence questions; future: follow-on jobs |
| **`dome.daily`** *(first-party)* | `shipped` | `daily`; future: `weekly` | — | `agenda-with.ts`, `ambiguous-followup-answer.ts`, `carry-forward.ts`, `create-daily.ts`, `prep.ts`, `task-index.ts`, `today.ts`; future: `create-weekly.ts`, `week-review.ts`, `append-followup.ts` | — | `read: ["wiki/**/*.md"]`; `patch.auto: ["wiki/**/*.md"]`; `graph.write: ["dome.daily.*"]`; `question.ask: true` |
