...
|---|---|---|---|---|
| `projection.db` | `facts` | processors with `graph.write` grants; shipped writers are `dome.graph.*`, `dome.daily.task-index`, and `dome.intake.capture-index` | [[wiki/specs/projection-store]] §"Tables — facts" | `graph.write:<namespace>` |
