...
|---|---|---|---|---|
| `projection.db` | `facts` | processors with `graph.write` grants; shipped writers are `dome.graph.*` and `dome.daily.task-index` | [[wiki/specs/projection-store]] §"Tables — facts" | `graph.write:<namespace>` |
