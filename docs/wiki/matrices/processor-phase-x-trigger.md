...
- Structurally true now:
  ...
  - **Answer dispatcher** (`runAnswerHandlers` in `src/engine/answers.ts`) — operational. `dome resolve` / `dome answer` records the answer row, then dispatches matching garden-phase answer handlers against the adopted snapshot. Handler effects route through normal garden effect routing; PatchEffects become garden sub-Proposals.
