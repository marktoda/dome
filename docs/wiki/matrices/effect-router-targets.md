...
| **DiagnosticEffect (severity: "block")** | Blocks adoption; emits `engine.adoption.blocked` | Recorded in `projection.db.diagnostics` as severity `error` (garden can't block adoption — it ran *after*); surfaced via `dome doctor --show diagnostics` | Same as garden — recorded as severity `error` |
...
| **QuestionEffect** | Recorded in `projection.db.questions`; surfaced via `dome doctor --show questions` and `dome query --questions` | Same | Rejected: `phase-mismatch` diagnostic |
