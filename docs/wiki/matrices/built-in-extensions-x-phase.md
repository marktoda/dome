...
| **`dome.health`** | shipped | — | `outbox-recovery-questions` (signal `outbox.changed`); `outbox-recovery-answer`; `quarantine-recovery-questions` (signal `quarantine.changed`); `quarantine-recovery-answer`; `orphan-run-recovery-questions` (cron `0 * * * *` — orphans are absence, not a store transition); `orphan-run-recovery-answer`; `report-card` (cron `22 5 * * 1` — Monday 05:22 weekly card, before the 05:30 brief); `trust-review` (cron `24 5 * * 1` — Monday trust ladder, acts on the 05:22 card's evidence: propose-mode config-diff promotions + owner-needed dormancy questions) | — |
| **`dome.claims`** | shipped | shipped: `index` (also emits `dome.claims.key-collision` diagnostics for same-page same-key/different-value contradictions) | shipped: `stamp` (claim-line anchor identity), `render-facts` (claim-rich pages get a `## Current facts` digest block) | shipped: `stale-claims` (read-only `dome stale-claims` staleness audit over claim `asOf` dates) |
