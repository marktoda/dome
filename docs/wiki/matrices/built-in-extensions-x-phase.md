...
|---|---|---|---|---|
| **`dome.markdown`** | shipped | `validate-wikilinks`; `normalize-frontmatter`; `lint-frontmatter`; `page-status` (supersession facts); `lint-supersession`; `broken-images`; `stale-dates`; `raw-immutable`; `core-size` (core-memory size budget) | `ambiguous-wikilink-answer`; `repair-wikilinks` (cron `5 5 * * *`); `render-index` (cron `15 5 * * *`, index-catalog render from `description:` frontmatter); `refresh-updated` (cron `0 5 * * *`); `attic-sweep` (cron `45 4 * * 0` — weekly, proposes archive-moves of dead-stub pages to `attic/`, `mode: "propose"` PatchEffect) | `orphan-pages` |
| **`dome.graph`** | shipped | `links`; `tag-index` | — | — |
| **`dome.health`** | shipped | — | `outbox-recovery-questions` (signal `outbox.changed`); `outbox-recovery-answer`; `quarantine-recovery-questions` (signal `quarantine.changed`); `quarantine-recovery-answer`; `orphan-run-recovery-questions` (cron `0 * * * *` — orphans are absence, not a store transition); `orphan-run-recovery-answer`; `report-card` (cron `22 5 * * 1` — Monday 05:22 weekly card, before the 05:30 brief); `trust-review` (cron `24 5 * * 1` — Monday trust ladder, acts on the 05:22 card's evidence: propose-mode config-diff promotions + self-clearing dormancy warning diagnostics) | — |
| **`dome.claims`** | shipped | shipped: `index` (also emits `dome.claims.key-collision` diagnostics for same-page same-key/different-value contradictions) | shipped: `stamp` (claim-line anchor identity), `render-facts` (claim-rich pages get a `## Current facts` digest block) | shipped: `stale-claims` (read-only `dome audit stale-claims` staleness audit over claim `asOf` dates) |
| **`dome.daily`** | shipped | shipped: `task-index` | shipped: `stamp-block-id` (block-anchor identity), `reconcile-tasks` (close-in-place propagation), `normalize-task-syntax` (task syntax hygiene), `create-daily` (cron `0 6 * * *`), `carry-forward` (daily open-loop surface), `close-scaffold` (cron `30 21 * * *`, evening close scaffold — schedule-only by design), `compose-blocks` (cron `25 5 * * *` + `questions.changed`/source-file/sweep-ledger signals, deterministic questions/agenda/integrated/sources block compile), `ambiguous-followup-answer`, `stale-task-warden` (cron `0 6 * * *`, structured settle-stale question per overdue-≥14-day task), `settle-stale-answer` (answer-triggered; applies close/defer/keep disposition to the task identified by ^anchor); planned: `create-weekly`, `append-followup` | shipped: `agenda-with`, `prep`, `today`; planned: `week-review` |
...
| **`dome.agent`** | shipped | shipped: `brief-index` (adoption signal `document.changed` on agent-fact files; emits `dome.agent.brief` fact from today's daily); `calendar-index` (adoption signal `document.changed` on calendar source files; emits `dome.agent.calendar.event` facts) | shipped: `ingest` (model-driven raw-capture agent loop), `inbox-stale-check` (cron `0 * * * *`), `consolidate` (cron `0 2 * * *`, nightly recent-drift janitor + knowledge-integrity review via the `flagIntegrity` tool, folded in from the retired `dome.warden`; proposes lossless page splits via `proposeSplit` — operation 4, `mode: "propose"` PatchEffect, the proposal-review loop's first producer), `sweep` (cron `0 3 * * *`, nightly meaning integration), `sweep-answer` (answer-triggered), `brief` (cron `30 5 * * *`, morning-brief composer), `preference-signals` (deterministic/rebuild-eligible preference counter facts), `preference-promotion` (deterministic owner-needed promotion questions), `preference-promotion-answer` (answer trigger; gated core.md writer — owns the promoted-preferences block), `active-projects` (cron `20 5 * * *`; deterministic gated core.md writer — owns the active-projects block), `patrol` (cron `45 1 * * *`; deterministic staleness selector — queues the 5 stalest entity/concept/synthesis pages into `meta/patrol-queue.md` for the 02:00 consolidate, bounded 60-day `meta/patrol-ledger.md`, `page.oversized` propose-split nudge) | — |
| **`dome.migrate`** | planned | — | — | `migrate-vault` |

## Counts

- **Shipped processors:** active processor modules across `dome.claims`, `dome.markdown`, `dome.graph`, `dome.health`, `dome.daily`, `dome.lint`, `dome.search`, `dome.sources`, and `dome.agent`.
