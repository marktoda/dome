...
| **`dome.health`** | shipped | — | `outbox-recovery-questions` (signal `outbox.changed`); `outbox-recovery-answer`; `quarantine-recovery-questions` (signal `quarantine.changed`); `quarantine-recovery-answer`; `orphan-run-recovery-questions` (cron `0 * * * *` — orphans are absence, not a store transition); `orphan-run-recovery-answer` | — |
| **`dome.claims`** | shipped | shipped: `index` (also emits `dome.claims.key-collision` diagnostics for same-page same-key/different-value contradictions) | shipped: `stamp` (claim-line anchor identity), `render-facts` (claim-rich pages get a `## Current facts` digest block) | shipped: `stale-claims` (read-only `dome run stale-claims` staleness audit over claim `asOf` dates) |
| **`dome.daily`** | shipped | shipped: `task-index` | shipped: `stamp-block-id` (block-anchor identity), `reconcile-tasks` (close-in-place propagation), `normalize-task-syntax` (task syntax hygiene), `create-daily` (cron `0 6 * * *`), `carry-forward` (daily open-loop surface), `close-scaffold` (cron `30 21 * * *`, evening close scaffold — schedule-only by design), `compose-blocks` (cron `25 5 * * *` + `questions.changed`/source-file/sweep-ledger signals, deterministic questions/agenda/integrated/sources block compile), `attention-discount` (dismissal-derived discount facts, deterministic/rebuild-eligible), `ambiguous-followup-answer`, `stale-task-warden` (cron `0 6 * * *`, structured settle-stale question per stale/overdue task), `settle-stale-answer` (answer-triggered; applies close/defer/keep disposition to the task identified by ^anchor); planned: `create-weekly`, `append-followup` | shipped: `agenda-with`, `prep`, `today`; planned: `week-review` |
