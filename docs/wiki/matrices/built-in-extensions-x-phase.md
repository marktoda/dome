...
| **`dome.health`** | shipped | — | `outbox-recovery-questions`, `quarantine-recovery-questions`, `orphan-run-recovery-questions` (each cron `*/5 * * * *`); `outbox-recovery-answer`; `quarantine-recovery-answer`; `orphan-run-recovery-answer` (answer triggers) | — |
