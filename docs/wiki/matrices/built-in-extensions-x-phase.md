...
| **`dome.intake`** | partially shipped | shipped: `capture-index`; planned: `inbox-stale-check` | shipped: `extract-capture`, `low-confidence-answer` | — |
| **`dome.migrate`** | planned | — | — | `migrate-vault` |

## Counts

- **Shipped processors:** 27 active processor modules across `dome.markdown`, `dome.graph`, `dome.health`, `dome.daily`, `dome.lint`, `dome.search`, and `dome.intake`.
