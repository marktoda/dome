...
| **`dome.health`** | shipped | — | `outbox-recovery-questions` (signal `outbox.changed`); `outbox-recovery-answer`; `quarantine-recovery-questions` (signal `quarantine.changed`); `quarantine-recovery-answer`; `orphan-run-recovery-questions` (cron `0 * * * *` — orphans are absence, not a store transition); `orphan-run-recovery-answer`; `report-card` (cron `22 5 * * 1` — Monday 05:22 weekly card, before the 05:30 brief) | — |
