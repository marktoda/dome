...
The runtime enforces the intersection of the declared and granted allowlists before any provider call. It also enforces the stricter defined `maxDailyCostUsd` from declaration and grant: before a provider call it denies when the cap is already spent; after a provider-reported cost is recorded it denies the output if that call pushed spend over the cap. The model handle shares the processor invocation signal, so processor timeout/cancellation aborts in-flight provider calls. Provider responses are validated inside the model boundary before processor code receives them. Structured output uses `ctx.modelInvoke.structured({ schemaName, parse })`; parse failures become nominal runtime-created `model.output.*` run errors rather than generic `processor.threw`. Processor-thrown or provider-thrown objects cannot opt into model execution codes by shape. Model-capable PatchEffects are additionally required to carry SourceRefs at the executor boundary so LLM-written vault changes are evidence-backed before capability routing.
