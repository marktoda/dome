...
- **`apply_proposal`** applies a pending garden-`propose`-mode edit's changes (writes and/or deletes) as one ordinary human commit via `performApply` — the settle pattern for proposals: every change is classified against the working tree vs the recorded base contents before anything is written (eligible / already-satisfied-delete / stale) and any stale change aborts the whole apply. A proposal that is entirely already-satisfied deletes lands no commit.
- **`reject_proposal`** CAS-decides a pending proposal row to `rejected` via `performReject`; touches no files.
- **`report_miss`** appends one grammar-exact bullet to `meta/retrieval-misses.md` (created with a header on first miss) via `reportMiss`, landing as one ordinary human commit (`miss: <query first 40 chars>`) — the same collector `dome query --miss` / `dome export-context --miss` call. This is the mechanical retrieval-miss-log channel [[memory]] §"M6 — Banked embeddings design (spec-only)" gates implementation of banked embeddings on; see [[wiki/specs/vault-layout]] §"`meta/retrieval-misses.md`".
...
- **No general document mutation.** No `write_document`, no `move_document`, no `delete_document`. External writes are git-native, and the targeted-edit exceptions are `settle` — a typed decision (close / defer / keep) on a task line located by its `^block-anchor`, not a free-form edit tool — and `apply_proposal`, which writes only the exact file contents a garden processor already proposed and vetted by capability check, not arbitrary caller-supplied content; both are commit-or-nothing seams, not free-form edit tools (`dome settle` / `POST /settle`; `dome apply` / `POST /apply`; [[wiki/specs/task-lifecycle]] §"The settle operation"). `tasks` and `proposals` stay list-only; settling a listed task goes through `settle`, and deciding a listed proposal goes through `apply_proposal` / `reject_proposal`, not through `tasks` / `proposals` themselves.
...
- [[wiki/specs/vault-layout]] §"`meta/retrieval-misses.md`" — the file the `report_miss` tool writes.
- [[memory]] §"M6 — Banked embeddings design (spec-only)" — the retrieval-miss-log gate `report_miss` operationalizes.
