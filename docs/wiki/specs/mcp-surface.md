...
| `dome.run_command` | `AbstractSurface.commands.<name>` | Invoke a view-phase command processor (`query` / `lint` / `export-context` / `today` / `prep` today; planned examples include `stats`). |
...
inputSchema:
  ...
  required: [name]
  properties:
    name:  { type: string, description: "Command processor name (query/lint/export-context/today/prep today; planned examples include stats)" }
