...
- **`apply_proposal`** applies a pending garden-`propose`-mode edit's changes as one ordinary human commit via `performApply` — the settle pattern for proposals: staleness (working tree vs the recorded base contents) is checked before anything is written, and delete-changes are rejected as `unsupported` in v1.
