...
| HTTP (shipped, `openVault`-consumer today) | `createDomeHttpServer(opts)` in `src/http/server.ts`, hosted by `dome http`; future `renderHttp(surface)` | Bearer-token JSON routes — capture/status/query/tasks/doc/questions/resolve per [[wiki/specs/http-surface]] |
