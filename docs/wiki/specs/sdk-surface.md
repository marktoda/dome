...
|---|---|---|
| `dome.markdown` | adoption: validate-wikilinks, normalize-frontmatter, lint-frontmatter, broken-images, duplicate-detection, stale-dates; view: orphan-pages | Keeps markdown pages well-formed; normalizes frontmatter, refreshes managed `updated:` dates during adoption, emits warning diagnostics on broken managed-page references/frontmatter and informational diagnostics for broken note-draft links and remaining adopted-date drift, asks about high-confidence duplicate canonical content pages, and provides the orphan-pages view. |
