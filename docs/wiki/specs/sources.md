...
The SDK ships TWO calendar fetch-command templates — vault-side data, not SDK code (the model-provider-template precedent: shipped executable vault-side data, never imported by `src/`). `assets/source-handlers/claude-calendar.sh` drives headless `claude -p` against the owner's claude.ai Google Calendar connector; it is a foreground-only reference (§"Connector-backed fetch is foreground-only"), not something to enable unmodified on a daemon. `assets/source-handlers/icalbuddy-calendar.sh` is **the deterministic macOS default for daemon subscriptions**: it reads Calendar.app directly via `icalBuddy` (EventKit), so it needs no interactive login session and a launchd/systemd-spawned daemon can run it unattended; it handles recurring events correctly, takes an optional `ICAL_CALENDARS` include-list env var to exclude noise calendars (empty means all), and still writes and commits an empty day's file rather than skipping it. A macOS TCC Calendar-access denial makes `icalBuddy` exit non-zero, which the template propagates — failing loud through the ordinary outbox retry → health-question path, no new signal needed.

`dome init --with-source calendar` scaffolds `claude-calendar.sh` (to `.dome/bin/fetch-calendar.sh`, executable, first-write-only) and ensures the disabled subscription stanza ([[wiki/specs/cli]] §"`dome init`"); an owner who wants a genuinely daemon-driven fetch copies `icalbuddy-calendar.sh` over it instead of hand-writing one (or adjusts the fetch line to `gcalcli`, EventKit, or a direct API call — whatever the vault can reach without an interactive session) and flips `enabled: true`. Both templates enforce the same command contract: write `sources/calendar/<date>.md` in the [[wiki/specs/vault-layout]] calendar-day shape, commit it with a pathspec-scoped commit, exit non-zero on any failure (so the outbox sees it) — and when the output file already exists, skip the fetch entirely and just commit it (the commit-only retry for a prior attempt whose commit failed).
