...
The SDK ships `assets/source-handlers/claude-calendar.sh` — a fetch-command template, not SDK code (the model-provider-template precedent: shipped executable vault-side data, never imported by `src/`). `dome init --with-source calendar` performs the copy (to `.dome/bin/fetch-calendar.sh`, executable, first-write-only) and ensures the disabled subscription stanza ([[wiki/specs/cli]] §"`dome init`"); the owner adjusts the fetch line (`icalBuddy`, `gcalcli`, EventKit, or a direct API call — whatever the vault can reach without an interactive session; see §"Connector-backed fetch is foreground-only" for why the shipped `claude -p` line is not that) and flips `enabled: true`. The template enforces the command contract: write `sources/calendar/<date>.md` in the [[wiki/specs/vault-layout]] calendar-day shape, commit it with a pathspec-scoped commit, exit non-zero on any failure (so the outbox sees it) — and when the output file already exists, skip the fetch entirely and just commit it (the commit-only retry for a prior attempt whose commit failed).
...
2. **Automated daemon fetch requires a DETERMINISTIC source** — one that needs no interactive login: `icalBuddy` reading Calendar.app or `gcalcli`/EventKit for calendar, a direct Calendar or Slack Web API call with a **file-stored token** for either. Anything that emits the day-file markdown shape on stdout without depending on a connector session works. The subscription machinery (consent re-check, outbox, retry, HEAD-verified completion, revocation) is unchanged — only the FETCH line must be swapped for a deterministic fetcher before a subscription is genuinely daemon-driven.
3. **Calendar joins Slack as a foreground ritual by default.** Live calendar and Slack come from the owner's morning foreground Claude session, where the connectors are loaded (the `/morning`-style rituals). The daemon-composed brief covers **vault state** — yesterday digest, open loops, questions — and gracefully omits the meetings and Slack surfaces when no day-file is present ([[wiki/specs/daily-surface]] §"The degradation ladder"; an absent source file means "not known", never a fabricated section). This generalizes the Slack stance below to calendar: both conversational connectors live in foreground rituals, and a daemon subscription for either is the deliberate deterministic-source path, not the shipped connector default.
...
What ships is the template plus the scaffolding, never the consent: `dome init --with-source slack` copies the template to `.dome/bin/fetch-slack.sh` and ensures the subscription stanza with `enabled: false` ([[wiki/specs/cli]] §"`dome init`" — scaffolding never flips an existing `enabled`). The template's default fetch is headless `claude -p` against the owner's claude.ai Slack connector — reading Slack **as the owner** — prompting for an overnight digest (mentions, DMs, high-traffic channels since the previous local evening) in the slack-day shape normative at [[wiki/specs/vault-layout]] §"`sources/slack/YYYY-MM-DD.md`". **But that connector-backed fetch is foreground-only** (§"Connector-backed fetch is foreground-only"): the claude.ai connector loads in an interactive session, not in the daemon's non-interactive `claude -p`, so the shipped Slack template is a foreground/example reference. The first reason below — interactive fetching stays in foreground rituals — is therefore not just a preference but the *only place the connector-backed default actually works*; a daemon-driven Slack subscription must swap the FETCH for a deterministic source (a direct Slack Web API call with a file-stored token). The **consent surface is the script plus the flip**: reading the prompt (which is the entire fetch) and setting `enabled: true` is what authorizes the Slack read; revocation is the same one-line flip as any subscription. The template validates before committing — first line `---`, frontmatter `date: <d>`, heading `# Slack <d>` — so a model refusal, an error page, or an empty fetch exits non-zero into the ordinary outbox retry instead of landing in the vault. Downstream, `dome.agent.brief` parses the committed digest defensively and injects it as data, never instructions ([[wiki/specs/autonomous-agents]] §"`dome.agent.brief`").
