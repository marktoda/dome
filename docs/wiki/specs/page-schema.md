...
`created:` is set once at page creation by the writer (processor, user, or scaffold). `updated:` is expected to match the date of the page's most recent committed content change. For managed `wiki/` pages with an existing `updated:` field, `dome.markdown.normalize-frontmatter` auto-patches drift greater than one day during adoption. `dome.markdown.stale-dates` remains a read-only informational rebuild/check diagnostic, and `dome.markdown.refresh-updated` clears adopted-state drift on a scheduled garden pass by writing the current run date so the maintenance commit itself becomes the new source of truth.

### Sources

`sources:` is optional but recommended. Carries a list of wikilinks pointing to evidence — typically raw captures (`[[raw/voice/2026-05-27-danny.md]]`) or other wiki pages. `dome.markdown.validate-wikilinks` emits warning diagnostics for unresolvable managed-page wikilinks, including broken `sources:` entries. When a broken managed-page link has multiple plausible existing targets, it asks an agent-safe source-backed question rather than guessing or creating a duplicate stub; answering that question with a candidate target patches only the original source span. When an unresolved managed link is an explicit missing `wiki/concepts/...` or `wiki/entities/...` path and no plausible existing page candidate exists, Dome may create a source-backed stub whose `sources:` cites the linking page. Pathful aliases that uniquely resolve by basename count as resolved and scheduled repair can canonicalize the target text. Bare links and user-owned `notes/` links do not create stubs; unresolved wikilinks authored under `notes/` or in imported `wiki/sources/` page bodies stay visible as informational diagnostics instead of release-gate attention.
