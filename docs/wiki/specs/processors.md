...
| `dome.agent` | garden: ingest, inbox-stale-check, consolidate, brief, preference-signals, preference-promotion, preference-promotion-answer | Runs autonomous-agent processors via a tool-use loop backed by `ctx.modelInvoke.step`; `dome.agent.ingest` handles `inbox/raw/*.md` — reads the raw source, creates/updates wiki pages (source, entities, concepts), wires bidirectional wikilinks, updates `index.md` and `log.md`, routes action-items to daily or entity surfaces, and archives the raw file — all as one `PatchEffect` (+ `QuestionEffect`s) with a single capability grant as the write boundary; `dome.agent.inbox-stale-check` emits `inbox.stale` warnings for files lingering under active inbox buckets past the 168-hour threshold; `dome.agent.consolidate` is the nightly recent-drift janitor and `dome.agent.brief` the morning-brief composer. The bundle's three deterministic preference processors (`preference-signals` counter facts, `preference-promotion` owner-needed questions, `preference-promotion-answer` — `core.md`'s single auto-writer) implement [[wiki/specs/preferences]]. See [[wiki/specs/autonomous-agents]]. |
...
| Scheduler | `schedule:` triggers fire on cron from `dome serve` and `dome sync` via the `projection.db.schedule_cursors` table; minimal in-tree cron evaluator (`src/engine/operational/cron.ts`); clock injection via `runOneAdoption({ now })` for deterministic harness testing | **Shipped** (Phase 4c) |
