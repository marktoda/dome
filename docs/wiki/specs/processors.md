...
|---|---|---|
| `dome.markdown` | adoption: validate-wikilinks, normalize-frontmatter, lint-frontmatter, broken-images, duplicate-detection, stale-dates, raw-immutable; garden: ambiguous-wikilink-answer; view: orphan-pages | Keeps markdown pages well-formed; normalizes frontmatter and refreshes managed `updated:` dates during adoption; emits DiagnosticEffect on ambiguous broken curated-page wikilinks, informational broken note-draft and imported-source body wikilinks, missing local image embeds, informational optional-root unknown page types, informational remaining stale adopted `updated:` dates, frontmatter issues, and raw-file mutations; handles common Obsidian link forms such as heading fragments, self-heading links, unique title-to-slug matches, and high-confidence typo repairs to existing pages without false broken-link warnings; asks agent-safe questions when a broken managed wikilink has multiple plausible existing repair targets and patches the chosen target when that question is answered; asks about high-confidence duplicate canonical content pages; provides the orphan-pages view. |
...
| `dome.search` | adoption: index-text; view: query, export-context | Maintains the FTS5 projection; answers adopted-state query and context-packet requests by ranking expanded FTS candidates with source-backed page-type, graph, open-loop, decision, question, and diagnostic signals, then rendering read-first overviews, open loops, decisions, questions, diagnostics, and source-backed match detail. |
