...
| `dome.search` | adoption: index-text; view: query, export-context | Maintains the FTS5 projection; answers adopted-state query and context-packet requests by ranking expanded FTS candidates with source-backed page-type, graph, open-loop, decision, question, and diagnostic signals. `export-context` also recalls pages whose projection memory has topic-matched open loops, decisions, questions, or diagnostics even when the page text itself misses FTS, then renders read-first overviews, recall signals, open loops, decisions, questions, diagnostics, and source-backed match detail. |
