...
| `dome.search` | adoption: index-text; view: query, export-context | Maintains the FTS5 projection; answers adopted-state query and context-packet requests by ranking expanded FTS candidates with source-backed page-type, graph, open-loop, decision, question, and diagnostic signals. Both query surfaces also recall exact-path documents whose projection memory has topic-matched open loops, decisions, questions, or diagnostics even when the page text itself misses FTS. `export-context` renders the richer handoff packet with read-first overviews, recall signals, topic-relevant open loops, decisions, questions, diagnostics, and source-backed match detail. |
