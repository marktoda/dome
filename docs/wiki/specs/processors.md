...
| `dome.intake` | garden: extract-capture | Compiles `inbox/raw/*.md` captures into generated capture pages plus processed archives, asks before tracking low-confidence items, and feeds high-confidence generated markdown into the daily task/followup index. |
