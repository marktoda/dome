...
|---|---|---|
| `dome.markdown` | adoption: validate-wikilinks, normalize-frontmatter, lint-frontmatter, broken-images, duplicate-detection, stale-dates, raw-immutable; view: orphan-pages | Keeps markdown pages well-formed; normalizes frontmatter and refreshes managed `updated:` dates during adoption; emits DiagnosticEffect on broken wikilinks, missing local image embeds, remaining stale adopted `updated:` dates, frontmatter issues, and raw-file mutations; asks about high-confidence duplicate canonical content pages; provides the orphan-pages view. |
