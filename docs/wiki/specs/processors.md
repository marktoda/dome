...
|---|---|---|
| `dome.markdown` | adoption: validate-wikilinks, normalize-frontmatter, lint-frontmatter, broken-images, duplicate-detection, stale-dates, raw-immutable; view: orphan-pages | Keeps markdown pages well-formed; normalizes frontmatter and refreshes managed `updated:` dates during adoption; emits DiagnosticEffect on broken managed-page wikilinks, informational broken note-draft wikilinks, missing local image embeds, informational remaining stale adopted `updated:` dates, frontmatter issues, and raw-file mutations; handles common Obsidian link forms such as heading fragments, self-heading links, and unique title-to-slug matches without false broken-link warnings; asks about high-confidence duplicate canonical content pages; provides the orphan-pages view. |
