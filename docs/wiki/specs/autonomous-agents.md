...
| `dome.agent.brief:compose-record` | under `## Start Here`, rendered last | the fingerprint-gate record: one italic line recording short content-hashes of the material inputs at last successful compose — `_Composed <n>× HH:MM · calendar@<hash8-or-dash> · slack@<hash8-or-dash> · ledger@<hash8-or-dash> · yesterday@<hash8-or-dash>_` (`<n>` = today's model-compose count so far, capped at 3; each `<hash8>` an 8-char content hash of that input, `—` when the input is absent). Written by every **successful** compose; the failure-stub path never writes it | processor (deterministic — never model-written) |
...
- **Garden projection read:** the brief reads projection facts (its stale-loops attention-discount context, below) through `ctx.projection`; it does not read `ctx.projection.questions()` — the open-questions batch left the brief's charter for `dome.daily.compose-blocks`, which reads via the separate `questions.read` operational capability instead ([[wiki/specs/capabilities]] §"`questions.read`"). The processor runtime threads the scoped read-only projection query view into **garden** contexts as well as view contexts (adoption stays snapshot-only for fixed-point determinism); see [[wiki/specs/processors]].
