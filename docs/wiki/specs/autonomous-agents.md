...
- **Phase / kind:** garden, `kind: llm`. **Triggers:** `schedule` (`30 5 * * *`) plus `file.created` signals on `sources/calendar/*.md` and `sources/slack/*.md` — the wake-tick late-source knock, gated deterministically (see "Late-source re-compose gate" below). The 05:30 cron path is unchanged by the gate.
...
| `dome.agent.brief:sources` | under `## Start Here`, rendered last | the sources-seen record: one italic line (`_Sources: calendar ✓ · slack —_`; ✓ = the source day-file existed at compose time, — = absent) naming which source day-files THIS compose saw. Written by every **successful** compose; the failure-stub path never writes it | processor (deterministic — never model-written) |
...
- **Late-source re-compose gate (deterministic, model-free):** a laptop wake tick can fire the brief in the same burst as the calendar/slack fetch dispatch — same-tick fires dispatch in cron-time order ([[wiki/specs/daily-surface]] §"Wake-tick choreography"), but the fetch completes asynchronously (outbox → command → commit → adoption), so a wake-tick brief can compose sourceless. The `file.created` signals above are the late arrival's knock. A signal-triggered run re-derives everything from TODAY's files (a backfilled past date naturally no-ops) and falls through to the normal compose **iff** a today-source file exists that today's daily's `dome.agent.brief:sources` record says the compose did not see. The record — not content inference — carries the state, because the meetings block's presence is a dishonest proxy (a present-but-empty calendar legitimately renders no meetings block) and the Slack digest leaves no mechanical footprint at all. **The no-op is free:** every other signal outcome returns zero effects and makes zero model calls — no daily or no parseable record means the brief has not successfully composed today (the 05:30 cron or a manual `dome run` owns the first compose, and a failed brief's recovery stays with its acknowledgeable question — signals never auto-retry a failure); all present sources recorded seen means the daily already reflects them. A re-compose records the source as seen, so the bound is at most one signal-triggered re-run per source kind per day.
...
- **Failure contract (roll-back-atomic, recover deterministically):** a mid-run throw means unknown partial state, so ALL of the model's edits roll back and nothing from the agent loop carries over. Recovery is effects-only and fully deterministic:
  ...
  2. A **fallback `PatchEffect`** splices a failure stub into the brief's own `dome.agent.brief:yesterday` block of the pre-run prepared content (the existing daily, or the freshly re-seeded skeleton when today's note was absent — so the day still starts with a complete deterministic daily rather than waiting for `create-daily` at 06:00). The stub names the flattened error (whitespace-collapsed, capped at 120 chars), links yesterday's note, and gives the retry command (`dome run dome.agent.brief`). Because the prepared content is deterministic, a same-day re-failure REPLACES the stub via the marker splice instead of appending a second copy; when the stub content equals the existing daily byte-for-byte, no patch is emitted. **Re-compose exception:** when today's daily already carries a successful compose (the `dome.agent.brief:sources` record is written only on success), the failure is a failed RE-compose — typically the late-source signal path — and NO fallback patch is emitted at all: the stub must not clobber the good yesterday/meetings bodies the morning compose landed (and a stub riding alongside them would misreport the morning as failed). The warning diagnostic and the brief-failed question carry the failure on their own.
