...
3. The agent invokes named operations via `Bash` when explicit structured ops are wanted: shipped examples are `dome query "..."`, `dome export-context "platform ownership"`, `dome status`, `dome inspect diagnostics`, and `dome run orphan-pages`; planned examples include `dome lint --apply H1` and `dome stats`.
...
- View-phase command processors invoked via the CLI (`dome query` and `dome export-context` today; planned aliases include `dome lint`).
