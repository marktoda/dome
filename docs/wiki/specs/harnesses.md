...
2. **The CLI** — shipped v1 primary commands are `dome serve`, `dome sync`, `dome status`, `dome check`, and `dome resolve`. Adopted-state recall views are `dome query`, `dome export-context <topic>`, and `dome today` (with its `--prep` planning and `--with <person-or-topic>` agenda framings); vault-consistency audits live under `dome audit <subject>` (`stale-claims`, `orphan-pages`). Advanced/debug and compatibility commands include `dome inspect <subject>`, `dome doctor`, `dome lint`, `dome answer`, `dome run <command>`, and `dome rebuild`. Planned user-value aliases such as `dome stats` should be treated as roadmap items unless the local CLI exposes them. The agent calls shipped commands via whatever shell-execution tool the harness exposes (`Bash` in Claude Code). See [[wiki/specs/cli]].
...
2. **A shell-execution capability** — so the agent can invoke shipped commands such as `dome sync`, `dome status`, `dome check`, `dome resolve`, `dome query`, and `dome export-context`. Claude Code's `Bash` is the reference implementation. Advanced commands such as `dome inspect`, `dome doctor`, `dome lint`, `dome answer`, and hidden daily view wrappers are available for debugging and compatibility. Planned aliases such as `dome stats` should not be assumed until Commander exposes them.
