...
- View-phase command processors invoked via the CLI (`dome query` today; planned aliases include `dome lint` and `dome export-context`).
...
2. **A shell-execution capability** — so the agent can invoke shipped commands such as `dome sync`, `dome status`, `dome query`, `dome inspect`, `dome doctor`, and `dome answer`. Claude Code's `Bash` is the reference implementation. Planned aliases such as `dome lint`, `dome stats`, and `dome export-context` should not be assumed until Commander exposes them.
