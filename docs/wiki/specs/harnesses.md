...
1. **`AGENTS.md` at the vault root** — auto-loaded by Claude Code (via `CLAUDE.md` shim) and any harness that supports the `AGENTS.md` convention. Teaches the agent the vault's conventions, core command surface, and invariants at session start. Generated by `dome init`; `dome init --refresh-instructions` refreshes the managed scaffold while preserving the delimited user-prose section. Bundle-local preamble merge remains planned v1.x work. See [[wiki/invariants/AGENTS_MD_IS_ORIENTATION_SURFACE]].
2. **The CLI** — shipped v1 primary commands are `dome serve`, `dome sync`, `dome status`, `dome check`, and `dome resolve`. `dome views --json` discovers the command-triggered views contributed by installed plugins; `dome query`, `dome export-context <topic>`, and `dome today` are first-party conveniences over that mechanism. Advanced/debug and compatibility commands include `dome inspect <subject>`, `dome doctor`, `dome lint`, `dome answer`, `dome run <command>`, and `dome rebuild`. The agent calls shipped commands via whatever shell-execution tool the harness exposes (`Bash` in Claude Code). See [[wiki/specs/cli]].
...
3. The agent invokes named operations via `Bash` when explicit structured ops are wanted. The normal operational path is `dome status --json` followed by its `next_actions` (`dome sync --json`, a `dome check ...` command, or `dome resolve <id> <value>`). Recall/context examples are `dome query "..."`, `dome export-context "platform ownership"`, and `dome audit orphan-pages`; advanced/debug examples include `dome inspect diagnostics` and `dome lint`.
...
- **Native mobile/web app** — uses deterministic HTTP operations for capture,
  status, decisions, document reads, and plugin views; conversation goes
  through the session protocol owned by an `AgentRuntime`. See
...
- **Web app** — the shipped PWA speaks the same deterministic HTTP + session
  protocol rather than importing engine internals.

New shells consume `Vault` in-process or wrap the shared `src/surface/`
operations over a transport. They do not require another aggregate surface
object. A shell that hosts conversation supplies an `AgentRuntime`; a shell
that brings its own agent can use native files, CLI, or MCP.
...
1. **`AGENTS.md` in the vault** (auto-generated by `dome init`; refreshable with `dome init --refresh-instructions` while preserving user prose) — the orientation surface.
2. **A shell-execution capability** — so the agent can invoke shipped commands such as `dome sync`, `dome status`, `dome check`, `dome resolve`, and `dome views`. Claude Code's `Bash` is the reference implementation. For a known page or bounded scope, the harness should use its native file tools directly; compiled query/context views help when the scope is unknown or cross-vault.
