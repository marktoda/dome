...
2. **The CLI** — shipped v1 primary commands are `dome serve`, `dome sync`, `dome status`, `dome check`, and `dome resolve`. Adopted-state recall views are `dome query` and `dome export-context <topic>`. Advanced/debug and compatibility commands include `dome inspect <subject>`, `dome doctor`, `dome lint`, `dome answer`, `dome run <command>`, `dome rebuild`, and hidden daily view wrappers. Planned user-value aliases such as `dome stats` should be treated as roadmap items unless the local CLI exposes them. The agent calls shipped commands via whatever shell-execution tool the harness exposes (`Bash` in Claude Code). See [[wiki/specs/cli]].
