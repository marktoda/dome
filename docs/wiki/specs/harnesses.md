...
3. The agent invokes named operations via `Bash` when explicit structured ops are wanted: shipped examples are `dome query "..."`, `dome lint`, `dome export-context "platform ownership"`, `dome status`, `dome inspect diagnostics`, and `dome run orphan-pages`; planned examples include `dome lint --apply H1` and `dome stats`.
...
- View-phase command processors invoked via the CLI (`dome query`, `dome lint`, and `dome export-context` today).
...
2. **A shell-execution capability** — so the agent can invoke shipped commands such as `dome sync`, `dome status`, `dome query`, `dome lint`, `dome export-context`, `dome inspect`, `dome doctor`, and `dome answer`. Claude Code's `Bash` is the reference implementation. Planned aliases such as `dome stats` should not be assumed until Commander exposes them.
