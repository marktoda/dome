...
3. The agent invokes named operations via `Bash` when explicit structured ops are wanted. The normal operational path is `dome status --json` followed by its `next_actions` (`dome sync --json`, a `dome check ...` command, or `dome resolve <id> <value>`). Recall/context examples are `dome query "..."`, `dome export-context "platform ownership"`, and `dome orphan-pages`; advanced/debug examples include `dome inspect diagnostics` and `dome lint`.
