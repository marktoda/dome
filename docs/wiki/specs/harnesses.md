...
2. **The CLI** — shipped v1 commands are `dome sync`, `dome serve`, `dome status`, `dome inspect <subject>`, `dome query`, `dome lint`, `dome export-context <topic>`, `dome today`, `dome prep`, `dome run <command>`, `dome rebuild`, `dome doctor`, and `dome answer`. Planned user-value aliases such as `dome lint --apply <id>` and `dome stats` should be treated as roadmap items unless the local CLI exposes them. The agent calls shipped commands via whatever shell-execution tool the harness exposes (`Bash` in Claude Code). See [[wiki/specs/cli]].
...
3. The agent invokes named operations via `Bash` when explicit structured ops are wanted: shipped examples are `dome query "..."`, `dome lint`, `dome export-context "platform ownership"`, `dome today`, `dome prep`, `dome status`, `dome inspect diagnostics`, and `dome run orphan-pages`; planned examples include `dome lint --apply H1` and `dome stats`.
...
2. **A shell-execution capability** — so the agent can invoke shipped commands such as `dome sync`, `dome status`, `dome query`, `dome lint`, `dome export-context`, `dome today`, `dome prep`, `dome inspect`, `dome doctor`, and `dome answer`. Claude Code's `Bash` is the reference implementation. Planned aliases such as `dome stats` should not be assumed until Commander exposes them.
