...
1. **`AGENTS.md` at the vault root** — auto-loaded by harnesses that support the `AGENTS.md` convention. It teaches the vault conventions, core command surface, and invariants at session start. Generated by canonical setup (including narrow `dome init` for new vaults); later owner changes are explicit. Bundle-local preamble merge remains planned v1.x work. See [[wiki/invariants/AGENTS_MD_IS_ORIENTATION_SURFACE]].
...
1. **`AGENTS.md` in the vault** (auto-generated by canonical setup; later changes are explicit owner edits) — the orientation surface.
