...
**MCP mounting is optional future work.** A user who wants the future MCP surface available (e.g., to test it once implemented, or in anticipation of a workflow that benefits from MCP-typed routing) would add the Dome MCP server to their Claude Code MCP config:
...
Once implemented and configured, Dome's MCP read/query tools and prompts are available — but the agent is not expected to reach for them when native tools suffice. See [[wiki/specs/mcp-surface]] for the target technical contract when MCP *is* mounted.
