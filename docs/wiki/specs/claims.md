...
| `dome.claims.stale-claims` | view | deterministic, read-only | Lists claims whose `*(as of)*` is older than `stale_claims_horizon_days` (default 120), computed at command time from the injected clock (`ctx.now()`) — a `ViewEffect`, never a persisted fact; invoked via `dome audit stale-claims`. |
...
`dome.claims.stale-claims` (`view`, invoked via `dome audit stale-claims`) reads
