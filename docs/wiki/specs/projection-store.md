...
5. On terminal failure (`attempts >= maxAttempts`), engine marks `status: "failed"`. Recovery follows the engine-asks model: failed rows surface through health/inspect; a `dome.health` question-emitter processor raises a `QuestionEffect` with options `["retry", "abandon"]`; the user answers via `dome answer <question-id>`; an answer-handler processor emits an `OutboxRecoveryEffect`; the engine-owned outbox sink applies the mutation. See [[wiki/specs/cli]] §"dome answer" and [[wiki/gotchas/outbox-stuck]]. The generic answer-handler + `OutboxRecoveryEffect` substrate ships now; the first-party `dome.health` question emitters and handlers remain v1 work.
