...
- `capabilityPolicyHash` — sha256 of the effective vault runtime policy and enabled-extension grants. Changing `.dome/config.yaml` or its create-only `.dome/content-scope.yaml` overlay in a way that alters the resolved policy invalidates the projection cache.
