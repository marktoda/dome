...
| 05:25 | `dome.daily.compose-blocks` | dome.daily | Deterministic block compile: questions / proposals / agenda / integrated / sources rendered from current inputs; also fires on `questions.changed` + `proposals.changed` + source-file + sweep-ledger signals all day. |
...
| `dome.daily:questions` | `## Start Here`, after the yesterday block | `dome.daily.compose-blocks` | deterministic — "To decide": **fresh** open questions (askedAt within `question_aging_days`, default **7**) sorted owner-needed first then oldest, top **3** shown, one-line text, recommended answer when present, literal `dome resolve <id> <value>` command, `+N more — dome check` tail when capped (fresh-only count); **aging** questions (askedAt ≥ `question_aging_days` ago) never render as bullets here — they collapse into one summary line before the end marker, `- 🕰 N aging decision(s) — weekly review (dome check --decisions)`, appended whenever any exist (all-fresh → no line; all-aging → heading with no bullets, just that line); *removed entirely* only when there are no open questions at all (fresh or aging) | 05:25 cron + `questions.changed` / source-file / sweep-ledger signals | Shipping. Replaces `dome.agent.brief:questions` (never rendered — free rename). Aging escalation is Task 10 — §"Question aging escalation". |
| `dome.daily:proposals` | `## Start Here`, after the questions block | `dome.daily.compose-blocks` | deterministic — "To review": top **3** pending garden proposals (oldest `createdAt` first), one-line reason, file count, literal `dome apply <id>` command, `+N more — dome proposals` tail when capped; *removed entirely* when no proposals are pending | 05:25 cron + `questions.changed` / `proposals.changed` / source-file / sweep-ledger signals | Shipping (product-review-4). Reads `ctx.operational.proposals({status: "pending"})`. |
...
An open question does not repeat forever in the "To decide" block. Once it has been open **≥ `question_aging_days`** (config key, default **7**), it stops rendering as a daily bullet and instead escalates to the weekly review — the daily nags for a bounded window, then hands the decision to the slower, more deliberate weekly surface instead of becoming permanent background noise.
...
**`dome.daily:questions` (the "To decide" block, `dome.daily.compose-blocks`).** Fresh questions render exactly as before this task shipped: owner-needed first, then oldest, capped at **3**, `+N more — dome check` counting fresh questions only (an aging question never occupies a slot or contributes to the tail count). When one or more aging questions exist, ONE summary line is appended before the end marker: `- 🕰 N aging decision(s) — weekly review (dome check --decisions)`. When every open question is aging, the block still renders its `### To decide` heading (so the block's own presence — "the page still has open decisions" — stays visible) but carries no question bullets, just that one line. The block is omitted entirely (as before) only when there are zero open questions, fresh or aging.

- `questionsSection(questions, opts?)` gained a second, optional parameter: `{ agingDays?: number; nowIso?: string }`. Omitting `opts` (or omitting `nowIso` specifically — this renderer is pure and has no clock of its own) reproduces the exact pre-Task-10 behavior: every question is treated as fresh. `compose-blocks.ts` always passes both, deriving `nowIso` from the same `ctx.now()`/`firedAt` fence the rest of the processor uses and `agingDays` from `question_aging_days` in `ctx.extensionConfig` (degrade-not-crash: missing, non-numeric, non-integer, or non-positive falls back to the default 7 — mirrors `minClaimsFromConfig` in `dome.claims/processors/render-facts.ts`).
...
|---|---|---|
| No model provider | The edition degrades to mechanical: `dome.daily.compose-blocks` still renders agenda + questions + proposals + integrated + sources at 05:25 from current inputs, the brief is a clean no-op (no error, no failed run — the warden no-op contract), `create-daily` writes the skeleton at 06:00 with the mechanical fallback body inside the unified `dome.agent.brief:yesterday` block (exactly one yesterday block, exactly once), and `carry-forward` raises the open-loops surface. The day still starts with a complete deterministic daily — agenda, questions, proposals, integrated, sources, open-loops, skeleton — a useful morning package with zero model. | `dome.daily.compose-blocks`, `dome.agent.brief` (no-op), `dome.daily.create-daily`, `dome.daily.carry-forward` |
...
| `proposals.read` declared but `ctx.operational.proposals` absent | A warning diagnostic `dome.daily.proposals-view-missing` fires and the "To review" block is omitted — loud, never a silent empty render (the same NEEDS_ARE_LOUD pattern as the questions rung above). | `dome.daily.compose-blocks` |
