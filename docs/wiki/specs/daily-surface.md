...
**Grant shape.** The processor uses a per-processor **replacement** grant (narrower than the `dome.health` recovery-processor bundle grant): `read`/`patch.auto` over `meta/report-card.md` + `wiki/dailies/*.md`, `run.read: true` (all statuses), `questions.read: true`. The manifest's `doctor.grantEntries` names the exact `patch.auto` YAML a pre-rollout vault must add so the write never silently drops.
