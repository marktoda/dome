...
This spec is normative for the daily note as a *product surface* — the one file where the system and the owner meet. The mechanism layer is specified elsewhere: [[wiki/specs/task-lifecycle]] owns block-anchor identity, [[wiki/specs/autonomous-agents]] owns the brief, and [[wiki/specs/semantic-gardening]] owns semantic maintenance. This spec owns the package: which sections exist, who writes each block, the overnight choreography, and how the morning edition degrades.
...
|---|---|---|---|
| 02:00 | `dome.agent.garden` | dome.agent | Compiles one evidence-backed semantic opportunity into an owner-reviewed proposal. |
...
| 05:22 (Mon) | `dome.health.report-card` | dome.health | **Weekly only** (cron `22 5 * * 1`). Deterministic weekly garden report: reads the run ledger (all statuses) + question rows over the trailing 7 days + proposal rows (the trust-ladder section) + the optional retrieval-miss log, rewrites `meta/report-card.md` and splices the `dome.health:report-card` block into today's daily under `## Weekly review` — in ONE PatchEffect. Ordered before the 05:25 compose and 05:30 brief so Monday's edition carries the week's what-ran/what-it-cost/what-got-acted-on. §"Report card". |
| 05:24 (Mon) | `dome.health.trust-review` | dome.health | **Weekly only** (cron `24 5 * * 1`). The trust ladder ([[wiki/specs/proposals]] §"Trust ladder"): acts on the card's evidence — proposes comment-preserving `.dome/config.yaml` promotions (mode:"propose" only, owner applies with `dome apply`) and raises self-clearing warning diagnostics for cost-with-no-output dormancy. Writes no daily block. |
| 05:25 | `dome.daily.compose-blocks` | dome.daily | Deterministic block compile: one canonical owner-attention block plus agenda and source presence; also fires on owner-attention and source-file signals. |
| 05:30 | `dome.agent.brief` | dome.agent | **The narrative compile.** Composes today's forward narrative, yesterday digest, and meetings prep. Its compose-record hashes calendar, Slack, and yesterday inputs; all-match is a zero-model no-op, capped at 3 model composes/day. |
...
2. **The brief re-composes when its inputs change.** Fetch completion is asynchronous, so a wake tick can compose before today's calendar or Slack file lands. The brief therefore also triggers on those files' creation. A deterministic pre-pass hashes calendar, Slack, and yesterday's daily against the compose record: all-match is a zero-model no-op; a mismatch recomposes all three narrative blocks, capped at three successful composes per day.
...
| `Start Here` | The first read of the morning — the edition's front page. | Shared: `dome.daily.compose-blocks` (deterministic blocks) + `dome.agent.brief` (narrative blocks) + optional human prose. | `dome.agent.brief:today`, `dome.agent.brief:yesterday`, `dome.daily:questions` (canonical owner attention), `dome.daily:sources`, `dome.agent.brief:compose-record`; retired legacy markers are recognized only for removal/anomaly reporting. | None — generated owner-attention and source blocks use plain bullets. |
...
| `Notes` | Free-form human capture. | Human. | None. (Ingest/capture-routed task lines land in `## Captured today`, not here.) | Because this is a daily note, any extracted checkbox/directive outside generated blocks, fences, and frontmatter is intentional daily work and feeds `task-index` / carry-forward. Outside dailies, a plain checkbox stays a local checklist unless explicitly marked; [[wiki/specs/task-lifecycle]] §"Global task semantics" is normative. |
...
| `Weekly review` | The week's garden report at a glance — Monday only. | Machine (`dome.health.report-card`, cross-bundle writer). | `dome.health:report-card` (§"Report card"). | None — plain-bullet digest; excluded from task extraction and search indexing. |
...
| `dome.daily:questions` | `## Start Here`, after the yesterday block | `dome.daily.compose-blocks` | deterministic — "Dome needs you": the bounded `dome.attention/v1` primary list, mixing genuine owner decisions and proposal reviews in the canonical rank and one shared cap of **3**. Agent-safe questions never appear. A `+N in owner backlog` tail accounts for aged, stale, or over-budget requests. Removed entirely when owner attention is empty. | 05:25 cron + `questions.changed` / `proposals.changed` / source-file signals | Shipping. The marker name remains for migration compatibility; semantics are owned by [[wiki/specs/owner-attention]]. |
| `dome.daily:proposals` | — | **None — retired-legacy.** | — | — | **Retired-legacy: recognized and removed, never written.** Reviews now share `dome.daily:questions`' one owner-attention budget. |
| `dome.daily:agenda` | `## Meetings`, top | `dome.daily.compose-blocks` | deterministic — time · title · attendees from `sources/calendar/<today>.md`, same defensive parser as the cockpit path; omitted entirely when no calendar file | 05:25 cron + `questions.changed` / source-file signals | Shipping. |
| `dome.daily:sources` | `## Start Here`, after owner attention | `dome.daily.compose-blocks` | deterministic — entries render only for source kinds whose day-file exists today; omitted when none exist | 05:25 cron + owner-attention/source signals | Shipping. |
| `dome.agent.brief:today` | `## Start Here` | `dome.agent.brief` | model-written forward narrative for today — a grounded paragraph oriented toward now (current focus, intent, today's constraints), distinct from the yesterday digest. Splice position: top of `## Start Here`, above the yesterday block. Omitted entirely when the model is unavailable (no fallback prose). `dome.agent.brief-index` (adoption extractor) reads this block on every adopted daily-note commit and emits a `dome.agent.brief` fact (`predicate: "dome.agent.brief"`) carrying the stripped plain-text body and a sourceRef to the block; the fact drives the brief panel in `dome today` and the PWA Today surface. | 05:30 (brief cron) | Shipping (CB-T5, CB-T6). |
...
| `dome.health:report-card` | `## Weekly review` | `dome.health.report-card` (cross-bundle writer; the id is registered in dome.daily's `DAILY_GENERATED_BLOCKS` for anomaly scanning + extraction exclusion, and in dome.search's strip list) | deterministic: total model cost, top-3 spenders with productive counts, questions opened/resolved, retrieval-miss count (only when the misses file exists), an "### Aging decisions" subsection (one `questionBullet`-style line with its `dome resolve` command per open question aging past `question_aging_days` — omitted entirely when none are aging), `Full card: [[meta/report-card]]`; never model-written | 05:22 **Monday** (schedule-only, weekly) | Shipping. The week's garden report — §"Report card"; aging section is Task 10 — §"Question aging escalation". |
...
The candidate set uses the same `openLoopSurfaceSources` compiler as `task-index`: all daily action items qualify, while outside dailies plain unchecked boxes remain page-local checklists until they carry `#task` / `#followup`, directive syntax, priority, or date intent. Generated blocks, frontmatter, and fenced examples never qualify. This shared seam keeps the global fact count and carry-forward semantics in lockstep.
...
`dome.health.report-card` is the product review's **prove** instrument: per-processor what-ran / what-it-cost / what-got-acted-on, rendered weekly into the owner's ritual surface. It is simultaneously the retire-or-keep instrument and the wedge metric made visible. Cron `22 5 * * 1` (Monday 05:22 — ahead of the 05:25 compose and 05:30 brief so Monday's edition already carries the week's numbers). Deterministic: NO model, NO facts, NO questions.

**One PatchEffect, two files.** Each run reads the run ledger (`run.read`, all statuses) + question rows (`questions.read`) + proposal rows (`proposals.read`, all statuses — the trust-ladder section) over the trailing 7 days, PLUS the full open-question backlog (`questionsView({resolved: false})` — not window-scoped, since a question opened long before this week's window can still be aging) for the "Aging decisions" section (Task 10 — §"Question aging escalation"), plus the optional retrieval-miss log, then emits ONE `PatchEffect` covering both files (atomic — never a half-written package):

1. `meta/report-card.md` — the full card, rewritten in place. A per-processor table over the trailing 7 days (runs, failures, quarantines, model cost USD, productive-outcome count), a questions-opened/resolved table by processor, an "### Aging decisions" section (one `questionBullet`-style bullet with its resolve command per aging open question, or `_No aging decisions this week._` when none), a "## Trust ladder" table (per proposal-producing processor in the window: granted autonomy — auto / propose / unknown when the config is unreadable — decided/applied counts by `decidedAt`, accept rate; the evidence surface for the 05:24 `dome.health.trust-review` actuator, [[wiki/specs/proposals]] §"Trust ladder"), an optional retrieval-miss line, and a "Possibly idle" section listing processors with ≥50 runs and zero productive outcomes.
2. Today's daily gains the `dome.health:report-card` block under `## Weekly review`: total model cost, top-3 spenders with productive counts, questions opened/resolved, retrieval-miss count, an "### Aging decisions" subsection (same bullet shape — omitted entirely, heading included, when there are no aging questions), `Full card: [[meta/report-card]]`. The skeleton is created when today's daily is absent, so compose-blocks/create-daily later no-op — the shared skeleton pattern.

**Byte-identical re-render is a no-op** (the deterministic gate): when neither file's content changes, no patch is emitted. The window-end date is derived from the schedule's `firedAt` (never wall-clock), so a same-day re-run is identical.
...
**Grant shape.** The processor uses a per-processor **replacement** grant (narrower than the `dome.health` recovery-processor bundle grant): `read`/`patch.auto` over `meta/report-card.md` + `wiki/dailies/*.md` (+ `read` over `.dome/config.yaml` for the trust-ladder autonomy column), `run.read: true` (all statuses), `questions.read: true`, `proposals.read: true`. The manifest's `doctor.grantEntries` names the exact `patch.auto`/`read` YAML a pre-rollout vault must add so the write never silently drops. A missing proposals view degrades to the empty trust-ladder fallback with a `dome.health.report-card-proposals-view-missing` warning; an unreadable config degrades each trust row's autonomy to `unknown`.

## Question aging escalation

An open owner request does not repeat forever in the immediate daily list. Once an ordinary, non-urgent request has been open **≥ `question_aging_days`** (config key, default **7**), the canonical attention compiler moves it to backlog. High-consequence or urgent decisions remain primary-eligible. The weekly report still renders aging decisions in detail.

**The rule, shared by both surfaces.** A question is "aging" when `askedAt < nowIso - question_aging_days` (strict `<` — a question exactly `question_aging_days` old is still fresh; it ages out on the next tick). `partitionQuestionsByAge` (`dome.daily/processors/edition-blocks.ts`) is the single implementation of this comparison; both consumers below call it with the same semantics, so a question can never be simultaneously "fresh" on one surface and silently absent from the other.

**`dome.daily:questions` (the owner-attention block, `dome.daily.compose-blocks`).** `compileAttention` receives all open questions and pending proposals, filters agent-assigned work, applies the shared rank/aging policy, and returns the same primary/backlog split used by Today, status, check, HTTP, MCP, and the SDK. The renderer only formats that already-ranked document.

**`dome.health:report-card` (the weekly review, `dome.health.report-card`).** The processor makes an ADDITIONAL, separate read beyond its usual 7-day-windowed questions-opened/resolved stats: `questionsView({resolved: false})`, the full open backlog, partitioned by the same rule and its OWN `question_aging_days` resolution (same key, same default, same degrade-not-crash — resolved from this processor's own `extensionConfig`, not shared state with `dome.daily.compose-blocks`; the two processors can be granted different config values without contradiction, though they default identically). The aging subset renders as one `questionBullet`-style line per question — text, options, recommendation, and its literal `dome resolve <id> <value>` command — in an "### Aging decisions" section on both `meta/report-card.md` (always present, falling back to `_No aging decisions this week._` when empty) and the daily `dome.health:report-card` block (omitted entirely, heading included, when empty — the compact daily digest only grows when there is something to escalate).

**Why the daily doesn't just keep showing it.** An owner-needed question that nobody has resolved after a week is not being *ignored* by the reader in the way a fresh one might be scrolled past — it has already failed to get attention on the surface designed for fast decisions. Repeating it forever there is exactly the kind of unbounded, ever-growing daily noise this surface's other caps (open-loops, edition-question, edition-proposal) all exist to prevent. The weekly review is a slower cadence built for exactly this: a bounded, once-a-week accounting of what still needs a human, with the same resolve command the daily always offered.
...
|---|---|---|
| No model provider | The edition degrades to mechanical: `dome.daily.compose-blocks` still renders agenda + owner attention + integrated + sources at 05:25 from current inputs, the brief is a clean no-op, `create-daily` writes the skeleton at 06:00 with the mechanical fallback body inside the unified `dome.agent.brief:yesterday` block, and `carry-forward` raises the open-loops surface. | `dome.daily.compose-blocks`, `dome.agent.brief` (no-op), `dome.daily.create-daily`, `dome.daily.carry-forward` |
| `questions.read` declared but `ctx.operational.questions` absent | A warning diagnostic `dome.daily.questions-view-missing` fires; the attention block compiles from proposal reviews only. | `dome.daily.compose-blocks` |
| `proposals.read` declared but `ctx.operational.proposals` absent | A warning diagnostic `dome.daily.proposals-view-missing` fires; the attention block compiles from owner decisions only. | `dome.daily.compose-blocks` |
| `run.read` declared but `ctx.operational.runs` absent | A warning diagnostic `dome.health.report-card-runs-view-missing` fires and NO patch is emitted — the existing report card is never overwritten with an empty one (NEEDS_ARE_LOUD). A missing `ctx.operational.questions` degrades to an omitted questions section (`dome.health.report-card-questions-view-missing`), card still renders from run data. | `dome.health.report-card` |
...
| Source day-file lands after the brief composed (the wake-tick race) | The `file.created` signal re-composes only when calendar/Slack/yesterday hashes differ, capped at 3 successful model composes per day. | `dome.agent.brief` |
...
| Brief run fails mid-flight | The model's edits roll back atomically, but the day is not abandoned: a deterministic fallback patch lands the pre-run prepared content (existing daily, or the re-seeded skeleton) with an honest availability stub in the `dome.agent.brief:yesterday` block — the flattened error, yesterday's link, and confirmation that deterministic sections remain available — plus a `dome.agent.brief-failed` warning. Operational failure is not an owner decision, so it creates no QuestionEffect and offers no inert retry instruction. A same-day re-failure replaces the stub via the marker splice, never duplicates it. When the daily already carries a successful compose record, a failed re-compose emits no fallback patch: good narrative stays and the diagnostic alone reports the failed refresh. Normative at [[wiki/specs/autonomous-agents]] §"`dome.agent.brief`". | `dome.agent.brief` |
...
- [[wiki/specs/semantic-gardening]] — nightly semantic maintenance and proposal settlement
