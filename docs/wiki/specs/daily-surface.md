...
| `dome.health:report-card` | `## Weekly review` | `dome.health.report-card` (cross-bundle writer; the id is registered in dome.daily's `DAILY_GENERATED_BLOCKS` for anomaly scanning + extraction exclusion, and in dome.search's strip list) | deterministic — ≤10 lines: total model cost, top-3 spenders with productive counts, questions opened/resolved, retrieval-miss count (only when the misses file exists), `Full card: [[meta/report-card]]`; never model-written | 05:22 **Monday** (schedule-only, weekly) | Shipping. The week's garden report — §"Report card". |
...
**One PatchEffect, two files.** Each run reads the run ledger (`run.read`, all statuses) + question rows (`questions.read`) over the trailing 7 days plus the optional retrieval-miss log, then emits ONE `PatchEffect` covering both files (atomic — never a half-written package):

1. `meta/report-card.md` — the full card, rewritten in place. A per-processor table over the trailing 7 days (runs, failures, quarantines, model cost USD, productive-outcome count), a questions-opened/resolved table by processor, an optional retrieval-miss line, and a "Possibly idle" section listing processors with ≥50 runs and zero productive outcomes.
2. Today's daily gains the `dome.health:report-card` block under `## Weekly review` (≤10 lines: total model cost, top-3 spenders with productive counts, questions opened/resolved, retrieval-miss count, `Full card: [[meta/report-card]]`). The skeleton is created when today's daily is absent, so compose-blocks/create-daily later no-op — the shared skeleton pattern.
