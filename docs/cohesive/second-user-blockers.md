...
- ~~The cockpit token rides in a query parameter (`/today?token=…`), visible in URLs and server logs.~~ Retired 2026-07-16: the duplicate HTML cockpit and query-token authentication path were deleted. Dome Home owns the browser surface at `/`; compatibility clients send the shared bearer only in `Authorization`.
