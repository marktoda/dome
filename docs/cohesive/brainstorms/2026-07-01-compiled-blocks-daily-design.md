...
| `dome.daily:sources` | Honest sources record: entries **only for source kinds whose day-file exists today**; block omitted entirely when none exist — never a perpetual `calendar — · slack —`. (Plan narrowing: a dome.daily processor cannot read dome.sources config, so file-presence is the whole rule.) | Replaces `dome.agent.brief:sources`; no longer the brief's re-compose gate (fingerprints replace that, §3) |
