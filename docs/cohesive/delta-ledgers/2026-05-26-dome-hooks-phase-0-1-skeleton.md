...
**Approved direction source:** [`docs/cohesive/brainstorms/2026-05-26-dome-hooks-v1-roadmap.md`](../brainstorms/2026-05-26-dome-hooks-v1-roadmap.md) §"Layer 1 — Capture & rhythm" + §"Extensibility-layer pressure-test findings" G1, G3, G4, G5 + [`docs/superpowers/plans/2026-05-26-dome-hooks-v1-roadmap-plan.md`](../../superpowers/plans/2026-05-26-dome-hooks-v1-roadmap-plan.md) §"Phase 0 — substrate enablement" + §"Phase 1 — ship dailies as the first extension bundle". User dispatch confirmed four sub-decisions: (a) scope is Phase 0 substrate + Phase 1 dailies bundle skeleton, no LLM compile-on-write (Phase 2 deferred); (b) task carry-forward semantics are **copy-with-backref** (yesterday keeps lines; today gets copies with `from [[wiki/dailies/<prev>]]` footer attribution); (c) bundle split is **split by lifecycle** (`dailies`, `aggregation`, `recall` as separate bundles — only `dailies` in this scope); (d) migration path is **`dome migrate-dailies` one-shot CLI command contributed by the dailies bundle** (not a top-level shipped SDK command).
