...
**Classification:** **Mixed.** Design-layer changes: (a) the **extension-bundle mechanism** lands as a new packaging convention over the existing 5-kind registration surface — `sdk-surface.md` gains a §"Extension bundles" section between §"Registration" and §"Tiered feature model" naming the directory shape (`<vault>/.dome/extensions/<bundle-name>/`), the `manifest.yaml` schema, the six-step bundle-load lifecycle (page-types merge → preamble fragment → workflows merge → hooks register → CLI commands register → tools register), the fail-loud collision semantics, and the "first-party bundles ship from `assets/extensions/`" convention; (b) the **`upsertSection` Tool** bumps the canonical catalog from seven Tools to eight — `sdk-surface.md` §"Tool catalog" gains the row, §"Tool signatures" gains the input/output shape, the matrix at `tool-invariant-enforcement.md` is implicitly extended (the matrix already enumerates by Tool name), and `MCP_TOOL_NAMES` will surface a `dome.upsert_section` entry; (c) **schedule-driven hooks** land as an extension to declarative-hook YAML — `hooks.md` §"Declarative" gains the `schedule:` field with cron syntax + the schedule-execution-model subsection naming the per-vault scheduler in `wireDispatcher` and the at-most-once-per-reconcile catch-up clamp, §"Adding a new hook" splits "three forms" into "four forms" (event-reactive declarative, schedule-driven declarative, programmatic v0.5.1+, bundle-contributed, shipped-default); (d) **AGENTS.md preamble fragments** extends the `AGENTS_MD_IS_ORIENTATION_SURFACE` invariant — the templated section now carries a `## Extension conventions` block where each loaded bundle's `preamble.md` renders as a subsection, regenerated by `dome doctor --repair` in bundle load order; (e) the **`dome run-hook` CLI command** lands as the 9th shipped command — `cli.md` gains a `## dome run-hook` section + the §"Implementation note" command-mapping table grows the row + the count prose updates from "Eight commands" to "Nine commands" + the §"Adding a new command" recipe gains a §"Bundle-contributed commands" subsection naming the `dailies` bundle's `migrate-dailies` as the canonical example; (f) the **`daily` and `weekly` page types** land as bundle-contributed types in `page-schema.md` §"Extension types" + the §"Extension types" preamble explains the two-path declaration (vault-local vs bundle-contributed) with provenance tracked internally as `source: "vault"` vs `source: "extension:<bundle>"`; (g) **`.dome/extensions/<bundle>/` directory** lands in `vault-layout.md` §"Vault root" tree + §"Git repository structure" (committed; bundles are vault identity) + §"Ownership rules" (bundle author owns; user copies in to install); (h) two new gotchas — `extension-bundle-load-order.md` (page-type/preamble collision semantics + hook-ID namespacing) and `scheduled-hook-idempotency.md` (schedule-driven hook catch-up clamp + the `idempotent:` declaration's narrower meaning for scheduled hooks); (i) one new behavior matrix — `extension-bundle-shape.md` (extension-bundle × five contribution kinds with rows for `dailies` shipped, `aggregation` / `recall` anticipated, `hello-world` test fixture). Implementation-layer changes are deferred to `cohesive:implement-cohesively`: the bundle loader at `src/extensions/loader.ts`, the `ManifestSchema` Zod definition, the `loadVaultConfig`/`buildBuiltinHookRegistry`/`wireDispatcher` extension to thread bundle contributions through `openVault`, the `buildAgentsMdTemplated` signature extension to accept preamble fragments + the `## Extension conventions` rendering, the `DeclarativeHookSchema` extension to accept a `schedule:` field with cron validation, the per-vault scheduler in `wireDispatcher` reading `.dome/state/scheduled.json`, the `src/tools/upsert-section.ts` Tool implementation + its registry entry + MCP_TOOL_NAMES extension + Zod schemas, the `src/cli/commands/run-hook.ts` command + Commander wiring + re-export + integration test + `hook.manual.invoked` event projection, the first-party `dailies` bundle shipped at `assets/extensions/dailies/` (manifest + page-types + preamble + create-daily / create-weekly workflows + their schedule-driven hooks + the `migrate-dailies` CLI command), the hello-world test fixture at `tests/fixtures/extensions/hello-world/`, the new lockstep tests (`extension-bundles-load.test.ts`, `scheduled-hooks.test.ts`, `tool-surface-shape.test.ts` extended for upsertSection, `agents-md-is-orientation-surface.test.ts` extended for preamble-fragment cases), and the substrate-doc enforcement-anchor follow-ups (the two new gotchas' `enforced_at:` test paths land alongside the implementation).

**Files:** 7 substrate files rewritten (5 specs — `sdk-surface.md`, `hooks.md`, `cli.md`, `vault-layout.md`, `page-schema.md`; 1 invariant — `AGENTS_MD_IS_ORIENTATION_SURFACE.md`; 1 substrate-map catalog — `docs/index.md`). 3 substrate files added (2 gotchas: `extension-bundle-load-order`, `scheduled-hook-idempotency`; 1 behavior matrix: `extension-bundle-shape`). Implementation work (deferred) is enumerated across §"Implementation work" below with stable IDs spanning Phase 0a (extension bundle loader), Phase 0b (AGENTS.md preamble fragments), Phase 0c (schedule field), Phase 0d (upsertSection Tool), Phase 0e (dome run-hook), Phase 1a (dailies bundle skeleton), Phase 1b–d (creator hooks + workflows), Phase 1e (migrate-dailies bundle-contributed CLI command), and Phase 1f (hello-world fixture + integration tests).

**Conceptual changes:** This rewrite establishes the **extension bundle** as a packaging convention over the existing 5-kind registration surface, closing the brainstorm's G3 pressure point — bundles let a single feature ship as a coherent unit of page-type + preamble + workflows + hooks + CLI commands rather than as five hand-threaded registrations. The bundle mechanism is *not* a new primitive: the four-concept core (Vault, Document, Tool, Hook) stays sealed; bundles compose registrations the substrate already supports. Adjacent to bundles, three substrate primitives land that the brainstorm's pressure-test G1/G4/G5 named: schedule-driven hooks (`schedule:` field with cron syntax, the per-vault scheduler, the at-most-once-per-reconcile catch-up clamp), manual hook invocation (`dome run-hook` synthesizing `hook.manual.invoked` events), and idempotent marker-delimited section updates (`upsertSection` Tool). The first-party `dailies` bundle is the proof case for the bundle mechanism — its successful load end-to-end (page type registered, preamble fragment in AGENTS.md, schedule-driven creator hooks firing, migrate-dailies CLI command appearing in `dome --help`) validates that the substrate-extension surface works for non-trivial features without the bundle mechanism becoming a substrate primitive itself. The substrate's pressure-3 question from the prior refactor — *"can a future contributor or agent predict the right answer for every common change shape, anchored to a structural fence?"* — extends here to: "can a future feature author ship a coherent multi-kind extension without modifying core?" The answer is yes, via the bundle mechanism this rewrite pins.

**Named invariants:** None added in this rewrite. The 16-invariant catalog stays unchanged. The `AGENTS_MD_IS_ORIENTATION_SURFACE` invariant doc is extended (statement + structural enforcement + test guarantee + operational notes) to cover preamble-fragment threading without adding a new invariant — the invariant's scope already covers the templated AGENTS.md surface; extension-bundle preambles are a templated-content category, not a new contract.

**Behavior matrices:** One added: `extension-bundle-shape.md` (extension-bundle × five contribution kinds, with rows for first-party shipped + anticipated + test-fixture bundles). The matrix is the substrate-pin for the bundle mechanism; the v0.5.1 extension to `tests/integration/matrix-coverage.test.ts` will parse it for the structural-fence assertion. Existing matrices are unchanged: `tool-invariant-enforcement.md` implicitly extends via its Tool-name enumeration (adding upsertSection to the matrix happens at the same time as the Tool implementation lands, not as a substrate edit in this rewrite); `consumer-surface.md` is unchanged because extension bundles don't add a new consumer shell; `event-types-and-payloads.md` will add the `hook.manual.invoked` event when the run-hook command implementation lands (an implementation-side matrix extension, not a design-side substrate edit).

**Gotchas:** Two new gotchas. `extension-bundle-load-order` (alphabetical load + fail-loud collision semantics; the page-type/preamble surfaces silently merge in load order absent collision; hook-IDs are namespaced as `<bundle>:<filename>`; `enforced_at:` points at the new `tests/integration/extension-bundles-load.test.ts` integration test). `scheduled-hook-idempotency` (the catch-up clamp + the narrower `idempotent:` semantic for scheduled hooks; `enforced_at:` points at the new `tests/integration/scheduled-hooks.test.ts` integration test). Both ship with `enforced_at_status: deferred` because the named tests are the implementation work, not pre-existing files.

**Semantic linters:** None added in this rewrite. The two existing semantic-linter specs (`no-retired-symbol-names`, `wrap-mutating-invoke-consumption`) stay unchanged.
...
- `hooks-adding-a-new-hook-four-forms` — §"Adding a new hook" "Three forms" → "Four forms" with new "Declarative schedule-driven form" inserted between "Declarative event-reactive form" and "Programmatic form (v0.5.1+)"; new "Bundle-contributed form" inserted before "Shipped-default form".
