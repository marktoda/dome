...
- **Trust ladder** — the gardener earns autonomy through the review loop: `dome.health.trust-review` proposes a comment-preserving `.dome/config.yaml` diff promoting a proposal producer to `patch.auto` once ≥8 of its proposals were decided in 28 days at ≥0.75 accept rate (owner applies with `dome apply`; never self-granted), and flags cost-with-no-output dormancy with a self-clearing warning diagnostic. [[wiki/specs/proposals]] §"Trust ladder"
- **Report card** — the weekly `dome.health.report-card` page (Monday, before the edition): per-loop run/cost/outcome accounting, aging decisions, and the trust column — the read-only evidence half of the trust-ladder pair (trust-review, two minutes later, is the actuator). [[wiki/specs/daily-surface]] §"Report card"
...
- **Semantic garden** — the nightly proposal-only loop that compiles current markdown into one bounded coherence opportunity. [[wiki/specs/semantic-gardening]]
...
- **Split proposal** — a semantic-garden proposal whose changes rewrite an accreted page as a hub plus 2–6 sub-pages, validated losslessly before enqueue. [[wiki/specs/semantic-gardening]]
...
- **settle** — three unrelated lifecycles share this verb: (1) a *task* is **settled** when its checkbox reaches `[x]`/`[-]` — `dome settle <block-id> <close|defer|keep>` is the direct disposition verb (one ordinary commit); (2) the sweep records **settlement** of a material→destination pair via a `sources:` frontmatter link; (3) a maintenance loop's **settlement rule** tells status/check how to decide the loop's goal currently holds.
