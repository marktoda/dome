...
- **Patrol** — the nightly deterministic staleness selector (`dome.agent.patrol`): scans readable markdown, rewrites `meta/patrol-queue.md` with the stalest eligible pages for consolidate's next pass, and raises info-severity `page.oversized` diagnostics. A selector, not a gardener — it never edits content pages. [[wiki/specs/autonomous-agents]] §"Patrol"
