...
- [[cohesive/runbooks/2026-07-home-pwa-acceptance]] — owner release checklist separating automated installed-Chromium evidence from signed Mac, iOS Safari, and accessibility gates.
...
- [[wiki/specs/task-lifecycle]] — `^block-anchor` line identity (move-stable, not body-hash); the deterministic `dome.daily` task processors; the `lastHumanChangedAt` freshness rule; and the projection-backed, exact-grouped `TaskBacklog.list` review read model.
