...
export function consolidateCharter(opts: {
  ...
  return [
    ...
    "- **Propose a split** — when the page is an accreted multi-document (several unrelated topics grown together under one page). You do NOT split or reorganize yourself; `askOwner` to propose it (\"`X` reads like N separate documents — split into …?\").",
