...
export class ContentExtractor {
  ...
  private extractMainContent(html: string): string {
    ...
    const mainContentSelectors = [
      ...
      {
        regex:
          /<div[^>]*(?:id|class)=["'](?:content|main|post|entry|blog-content)[^>]*>([\s\S]*?)<\/div>/i,
        priority: 8,
      },
      {
        regex:
          /<div[^>]*(?:id|class)=["'](?:main-content|page-content|article-content|entry-content|post-content)[^>]*>([\s\S]*?)<\/div>/i,
        priority: 7,
      },
      {
        regex:
          /<section[^>]*(?:id|class)=["'](?:content|main|post|entry)[^>]*>([\s\S]*?)<\/section>/i,
        priority: 6,
      },
  ...
  private cleanContent(html: string): string {
    ...
    content = content.replace(
      /<div[^>]*(?:id|class)=["'](?:sidebar|widget|ads|advertisement|social|share|related|recommended)[^>]*>[\s\S]*?<\/div>/gi,
      '',
    );
