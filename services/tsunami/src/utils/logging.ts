...
export const tsunamiMetrics = createServiceMetrics('tsunami');

/**
 * Get a logger instance with the tsunami service name
 */
export function getLogger(): any {
  return getDomeLogger().child({ service: 'tsunami' });
}
...
export function logError(error: unknown, message: string, context: Record<string, any> = {}): void {
  ...
  tsunamiMetrics.trackOperation('error', false, {
...
export async function trackOperation<T>(
  ...
  return domeTrackOperation(operationName, fn, { service: 'tsunami', ...context });
}
