...
import { ValidationError, NotFoundError, ServiceUnavailableError, toDomeError } from '@dome/errors';
  ...
  async getIgnorePatterns(owner: string, repo: string): Promise<string[]> {
    ...
    return trackOperation(
      ...
      async () => {
        ...
        assertValid(repo && repo.trim().length > 0, 'Repository name cannot be empty', {
          owner,
          repo,
        });
          ...
          if (!response.ok) {
            ...
            if (response.status >= 500) {
              ...
              throw new ValidationError(
                `GitHub API rate limit or authentication error: ${response.statusText}`,
                {
                  owner,
                  repo,
                  status: response.status,
