...
describe('NotionAuthManager', () => {
  ...
  describe('getAuthUrl', () => {
    it('should generate valid Notion OAuth URL', () => {
      ...
      expect(parsedUrl.origin + parsedUrl.pathname).toBe('https://api.notion.com/v1/oauth/authorize');
  ...
  describe('exchangeCodeForToken', () => {
    ...
    it('should throw ServiceError on API error response', async () => {
      ...
      await expect(authManager.exchangeCodeForToken(code))
  ...
  describe('token storage and retrieval', () => {
    ...
    it('should handle errors in token storage', async () => {
      ...
      const mockTokenStore = {
        set: vi.fn().mockImplementation(() => { throw new Error('Storage error'); }),
        get: vi.fn()
      };
      (authManager as any).tokenStore = mockTokenStore;
      
      await expect(authManager.storeUserToken(userId, workspaceId, token))
        .rejects.toThrow(ServiceError);
    });
    ...
    it('should handle errors in token retrieval', async () => {
      ...
      const mockTokenStore = {
        set: vi.fn(),
        get: vi.fn().mockImplementation(() => { throw new Error('Retrieval error'); })
      };
      (authManager as any).tokenStore = mockTokenStore;
      
      await expect(authManager.getUserToken(userId, workspaceId))
        .rejects.toThrow(ServiceError);
    });
  });
