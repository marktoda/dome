...
export async function verifyGitHubWebhook(
  ...
  const expectedSignature = await crypto.subtle.sign('HMAC', key, encoder.encode(payloadString));
...
export async function handleWebhook(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    const isValid = await verifyGitHubWebhook(payload, signature, env.GITHUB_WEBHOOK_SECRET);
