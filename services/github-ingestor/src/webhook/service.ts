...
export class WebhookService {
      ...
      for (const repo of repoResult.results) {
        ...
        await this.enqueueRepositoryJob(
          ...
          repo.includePatterns
            ? (JSON.parse(repo.includePatterns as string) as string[])
            : undefined,
          repo.excludePatterns
            ? (JSON.parse(repo.excludePatterns as string) as string[])
            : undefined,
        );
  ...
  private async storeInstallation(userId: string, installationId: string): Promise<void> {
    ...
    if (existingInstallation.results.length > 0) {
      logger().info({ userId, installationId }, 'Installation already exists in database');
      return;
    }
    ...
    `,
      ...
      .bind(ulid(), userId, 'github', installationId, now, now)
      .run();

    logger().info({ userId, installationId }, 'Added installation to database');
  }
  ...
  private async removeInstallation(userId: string, installationId: string): Promise<void> {
    ...
    logger().info({ userId, installationId }, 'Removed installation from database');
  }
  ...
  private async addRepository(
    ...
    if (existingRepo.results.length > 0) {
      logger().info({ owner, repo, userId }, 'Repository already exists in database');
      return;
    }
    ...
    logger().info({ owner, repo, userId }, 'Added repository to database');
  }
   ...
   * @param repo Repository name
   */
  private async removeRepository(userId: string, owner: string, repo: string): Promise<void> {
    await this.env.DB.prepare(
      `
    ...
    logger().info({ owner, repo, userId }, 'Removed repository from database');
  }
}
