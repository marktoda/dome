...
export class ContentService {
  ...
  async storeContent(
    ...
    try {
      ...
      const r2Key = metadata.r2Key || generateR2Key(sha, metadata.mimeType || 'application/octet-stream');
      ...
      if (!response.ok) {
        throw new Error(`Failed to store content in Silo: ${response.status} ${response.statusText}`);
      }
  ...
  async getContent(sha: string): Promise<globalThis.ReadableStream> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to get content from Silo: ${response.status} ${response.statusText}`);
      }
  ...
  async deleteContent(sha: string): Promise<boolean> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to delete content from Silo: ${response.status} ${response.statusText}`);
      }
