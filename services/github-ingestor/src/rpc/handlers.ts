...
export class RpcHandlers {
  ...
  async getRepositoryStatus(
    ...
    try {
      ...
      if (result.retryCount > 0 && result.nextRetryAt && result.nextRetryAt > now) {
        ...
        status = 'rate_limited';
        error = `Rate limited, will reset at ${new Date(
          result.rateLimitReset * 1000,
        ).toISOString()}`;
      }
  ...
  async addInstallation(request: InstallationRequest): Promise<InstallationResponse> {
    ...
    try {
      ...
      `,
        ...
        .bind(id, userId, 'github', installationId, now, now)
        .run();
  ...
  async listInstallations(request: ListInstallationsRequest): Promise<InstallationResponse[]> {
    ...
    try {
      ...
      logError(
        error as Error,
        `Failed to list GitHub App installations for user ${request.userId} via RPC`,
      );
      throw error;
    }
  }
