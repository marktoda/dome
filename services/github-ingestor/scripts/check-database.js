...
function getDatabaseName() {
  try {
    ...
    const nameMatch = wranglerContent.match(/\[\[d1_databases\]\]\s+database_name\s*=\s*["']([^"']+)["']/);
...
async function checkSchema(dbName) {
  ...
  const result = runD1Command(`execute ${dbName} --command "SELECT name, sql FROM sqlite_master WHERE type='table'"`);
...
async function checkTableData(dbName, table, limit = 5) {
  ...
  const result = runD1Command(`execute ${dbName} --command "SELECT * FROM ${table} LIMIT ${limit}"`);
...
async function runDiagnostics() {
  ...
  console.log(`3. Consider running migrations manually with 'wrangler d1 migrations apply ${dbName}'`);
