...
export class TelegramClientPool {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new TelegramError(`Failed to initialize client pool: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async acquireClient(
    ...
    try {
      ...
      logger.info(`No clients available, queueing request${session ? ` for session ${session.id}` : ''}`);
      ...
      throw new TelegramError(`Failed to acquire client: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async createClient(): Promise<TelegramClientWrapper> {
    ...
    try {
      ...
      throw new TelegramError(`Failed to create client: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async performMaintenance(): Promise<void> {
    ...
    if (neededClients > 0) {
      ...
      for (let i = 0; i < neededClients; i++) {
        creationPromises.push(this.createClient().catch(error => {
    ...
    // Log pool status
    logger.info(`Pool status: ${this.clients.size} total, ${availableCount} available, ${this.requestQueue.length} waiting`);
  }
  ...
  private async processNextRequest(): Promise<void> {
    ...
    try {
      ...
      request.reject(new TelegramError(`Failed to process queued request: ${error instanceof Error ? error.message : 'Unknown error'}`));
