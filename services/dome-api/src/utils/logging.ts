...
export const domeApiMetrics = createServiceMetrics('dome-api');

/**
 * Get a logger instance with the dome-api service name
 */
export function getLogger(): any {
  return getDomeLogger().child({ service: 'dome-api' });
}
...
export function logError(error: unknown, message: string, context: Record<string, any> = {}): void {
  ...
  domeApiMetrics.trackOperation('error', false, {
...
export async function trackOperation<T>(
  ...
  return domeTrackOperation(operationName, fn, { service: 'dome-api', ...context });
}
