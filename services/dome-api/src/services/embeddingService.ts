...
export class EmbeddingService {
  ...
  async generate(env: Bindings, text: string): Promise<number[]> {
    ...
    try {
      ...
      throw new ServiceError('Embedding dimension mismatch', { context: { expected: this.dimension, got: vec?.length } })
    } catch (err) {
      throw new ServiceError('Failed to generate embedding', { cause: err instanceof Error ? err : new Error(String(err)) })
    }
  }
  ...
  splitTextIntoChunks(text: string, maxChunk = 2048): string[] {
    ...
    for (const p of paras) {
      if (cur.length + p.length > maxChunk && cur) { chunks.push(cur.trim()); cur = '' }
