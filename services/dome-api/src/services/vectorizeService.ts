...
export class VectorizeService {
  ...
  async addVector(env: Bindings, id: string, vector: number[], metadata: VectorMetadata) {
    try {
      ...
      throw err('addVector failed', { id }, e);
    }
  }
  ...
  async updateVector(env: Bindings, id: string, vector: number[], metadata: VectorMetadata) {
    try {
      ...
      throw err('updateVector failed', { id }, e);
    }
  }

  async deleteVector(env: Bindings, id: string) {
    try {
      await this._delete(env, id);
    } catch (e) {
      throw err('deleteVector failed', { id }, e);
    }
  }
  ...
  async queryVectors(
    ...
    try {
      ...
      throw err('queryVectors failed', { options }, e);
    }
  }
  ...
  async listVectors(
    ...
    try {
      ...
      return {
        vectors: matches.map(m => ({ id: m.id, metadata: m.metadata as VectorMetadata })),
        cursor: undefined,
      };
    } catch (e) {
      throw err('listVectors failed', { filter, options }, e);
    }
  }
