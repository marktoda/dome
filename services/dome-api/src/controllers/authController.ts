...
const LoginBodySchema = z.object({
  email: z.string().email().openapi({ example: 'mark@example.com', description: 'User\'s email address' }),
  password: z.string().min(8).openapi({ example: 'hunter2', description: 'User\'s password (min 8 characters)' })
}).openapi('LoginBody');

const LoginResponseSchema = z.object({
  token: z.string().openapi({ example: 'eyJhbGciOiJIUzI1NiIs…', description: 'JWT authentication token' })
...
const RegisterBodySchema = z.object({
  email: z.string().email().openapi({ example: 'newuser@example.com', description: 'New user\'s email address' }),
  password: z.string().min(8).openapi({ example: 'securepassword123', description: 'New user\'s password (min 8 characters)' }),
...
const RegisterResponseSchema = z.object({
  token: z.string().openapi({ example: 'eyJhbGciOiJIUzI1NiIs…', description: 'JWT authentication token after registration' })
...
const registerRoute = createRoute({
  ...
  responses: {
    201: { // Typically 201 Created for new resources
...
export class AuthController {
  ...
  register = async (
    ...
    try {
      ...
      if (result.error?.code === 'USER_ALREADY_EXISTS') { // Assuming this specific code from service
        this.logger.warn({ email, errorCode: result.error.code }, 'Registration attempt for existing user');
        return c.json({ success: false as const, error: { code: 'CONFLICT', message: result.error.message || 'User already exists' } }, 409);
     }

     this.logger.error({ result }, 'Registration service call failed, did not return token, or returned unexpected error');
     return c.json(
       { success: false as const, error: { code: 'INTERNAL_SERVER_ERROR', message: result.error?.message || 'Registration failed to produce a token or encountered an unknown issue.' } },
       500
     );
   } catch (error: any) {
     this.logger.error({ errorMessage: error.message, stack: error.stack, code: error.code }, 'Registration failed with exception');
     // If the caught error has a structure that indicates a specific business logic failure (e.g. user exists)
     if (error.code === 'USER_ALREADY_EXISTS') { // Check if service throws custom errors
        return c.json({ success: false as const, error: { code: 'CONFLICT', message: String(error.message) || 'User already exists' } }, 409);
     }
     return c.json(
       { success: false as const, error: { code: 'BAD_REQUEST', message: String(error.message) || 'Registration processing error' } },
       400
     );
   }
 };
  ...
  login = async (
    ...
    try {
      ...
      return c.json(
        { success: false as const, error: { code: 'UNAUTHORIZED', message: String(result.error?.message) || 'Invalid credentials' } },
        401
      );
    } catch (error: any) {
      this.logger.error({ errorMessage: error.message, stack: error.stack }, 'Login failed with exception');
      return c.json(
        { success: false as const, error: { code: 'UNAUTHORIZED', message: String(error.message) || 'Login processing error' } },
        401
      );
    }
  };
  ...
  logout = async (
    ...
    try {
      ...
      return c.json(
        { success: false, error: { code: 'INTERNAL_SERVER_ERROR', message: String(result.error?.message) || 'Logout operation failed' } },
        500
      );
    } catch (error: any) {
      this.logger.error({ errorMessage: error.message, stack: error.stack }, 'Logout failed with exception');
      return c.json(
        { success: false, error: { code: 'INTERNAL_SERVER_ERROR', message: String(error.message) || 'Logout processing error' } },
        500
      );
    }
  };
    ...
    try {
      ...
      if (result.success && result.user) {
        ...
        if (!validation.success) {
          this.logger.error({ errors: validation.error.flatten(), userFromService: result.user }, 'User profile from authService failed Zod validation against UserProfileSchema');
          return c.json({ success: false, error: { code: 'INTERNAL_SERVER_ERROR', message: 'Invalid user data structure received from authentication service.' } }, 500);
        }
        return c.json({ success: true, user: validation.data }, 200);
      }
      this.logger.warn({ result }, 'Token validation service call failed or did not return user profile');
      return c.json(
        { success: false, error: { code: 'UNAUTHORIZED', message: String(result.error?.message) || 'Invalid or expired token' } },
        401
      );
    } catch (error: any) {
      this.logger.error({ errorMessage: error.message, stack: error.stack }, 'Token validation failed with exception');
      if (error instanceof z.ZodError) { // This case should ideally be caught by the safeParse above.
        this.logger.error({ errors: error.flatten() }, 'UserProfileSchema validation failed unexpectedly during catch block.');
        return c.json({ success: false, error: { code: 'INTERNAL_SERVER_ERROR', message: 'User data structure error after validation.' } }, 500);
      }
      return c.json(
        { success: false, error: { code: 'UNAUTHORIZED', message: String(error.message) || 'Token validation processing error' } },
        401
      );
    }
  };
