...
const WorkspaceIdParamSchema = z.object({
  workspaceId: z
    .string()
    .openapi({ param: { name: 'workspaceId', in: 'path' }, example: 'notion_ws_123' }),
});

// --- Body Schemas ---
const RegisterNotionWorkspaceBodySchema = z
  .object({
...
const ConfigureNotionOAuthBodySchema = z
  .object({
    ...
    redirectUri: z
      ...
      .url()
      .optional()
      .openapi({ example: 'https://myapp.com/oauth/callback/notion' }),
  })
  .openapi('ConfigureNotionOAuthBody');

const StoreNotionIntegrationBodySchema = z
  .object({
...
// --- Response Schemas ---
const NotionWorkspaceResponseSchema = z
  .object({
...
const NotionSyncHistoryItemSchema = z
  .object({
...
const NotionSyncHistoryResponseSchema = z
  .array(NotionSyncHistoryItemSchema)
  .openapi('NotionSyncHistoryResponse');

const NotionOAuthUrlResponseSchema = z
  .object({
    url: z
      .string()
      .url()
      .openapi({ example: 'https://api.notion.com/v1/oauth/authorize?client_id=...' }),
  })
...
const registerNotionWorkspaceRoute = createRoute({
  ...
  request: {
    body: {
      content: { 'application/json': { schema: RegisterNotionWorkspaceBodySchema } },
      required: true,
    },
  },
  responses: {
    201: {
      ...
      content: { 'application/json': { schema: NotionWorkspaceResponseSchema } },
    },
    400: {
...
const getNotionWorkspaceHistoryRoute = createRoute({
  ...
  responses: {
    200: {
      ...
      content: { 'application/json': { schema: NotionSyncHistoryResponseSchema } },
    },
    401: {
      description: 'Unauthorized.',
      content: { 'application/json': { schema: ErrorResponseSchema } },
...
const configureNotionOAuthRoute = createRoute({
  method: 'post',
  path: '/oauth/configure', // Changed path to avoid conflict with /oauth/store or /oauth/url
  summary: 'Configure Notion OAuth Credentials (Internal/Admin)',
  security: [{ BearerAuth: [] }], // Likely admin restricted
  request: {
    body: {
      content: { 'application/json': { schema: ConfigureNotionOAuthBodySchema } },
      required: true,
    },
  },
  responses: {
...
const getNotionOAuthUrlRoute = createRoute({
  ...
  responses: {
    200: {
      ...
      content: { 'application/json': { schema: NotionOAuthUrlResponseSchema } },
    },
    401: {
      description: 'Unauthorized.',
      content: { 'application/json': { schema: ErrorResponseSchema } },
...
const storeNotionIntegrationRoute = createRoute({
  ...
  request: {
    body: {
      content: { 'application/json': { schema: StoreNotionIntegrationBodySchema } },
      required: true,
    },
  },
  responses: {
    200: {
      ...
      content: { 'application/json': { schema: GenericSuccessResponseSchema } },
    }, // Or NotionWorkspaceResponseSchema
    400: {
      description: 'Bad Request.',
      content: { 'application/json': { schema: ErrorResponseSchema } },
    },
  ...
  registerNotionWorkspace = async (
    ...
    try {
      ...
      return c.json(
        {
          success: false as const,
          error: { code: 'INTERNAL_SERVER_ERROR', message: String(error.message) || 'Failed' },
        },
        500,
      );
    }
...
export function buildNotionContentRouter(): OpenAPIHono<
...
> {
  ...
  router.openapi(registerNotionWorkspaceRoute, c =>
    notionController.registerNotionWorkspace(c, c.req.valid('json')),
  );
  router.openapi(getNotionWorkspaceHistoryRoute, c =>
    notionController.getNotionWorkspaceHistory(c, c.req.valid('param')),
  );
  router.openapi(triggerNotionWorkspaceSyncRoute, c =>
    notionController.triggerNotionWorkspaceSync(c, c.req.valid('param')),
  );
  router.openapi(configureNotionOAuthRoute, c =>
    notionController.configureNotionOAuth(c, c.req.valid('json')),
  );
  router.openapi(getNotionOAuthUrlRoute, c => notionController.getNotionOAuthUrl(c));
  router.openapi(storeNotionIntegrationRoute, c =>
    notionController.storeNotionIntegration(c, c.req.valid('json')),
  );
