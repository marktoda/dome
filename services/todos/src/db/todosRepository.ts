...
export class TodosRepository {
  ...
  async createTodo(input: CreateTodoInput): Promise<TodoItem> {
    ...
    const actionableSteps = input.actionableSteps
      ? JSON.stringify(input.actionableSteps)
      : undefined;
  ...
  async getTodo(id: string): Promise<TodoItem | null> {
    try {
      const results = await this.db.select().from(todos).where(eq(todos.id, id)).limit(1);
      ...
      if (search) {
        ...
        conditions.push(
          or(
            ...
            and(isNotNull(todos.description), like(todos.description, `%${search}%`)),
          ),
        );
      }
    ...
    try {
      ...
      if (updates.status !== undefined) {
        ...
        if (
          ...
          currentTodo.status !== TodoStatus.COMPLETED
        ) {
      ...
      if (updates.actionableSteps !== undefined) {
        updateValues.actionableSteps = updates.actionableSteps.length
          ? JSON.stringify(updates.actionableSteps)
          : null;
      }
      ...
      const result = await this.db.update(todos).set(updateValues).where(eq(todos.id, id));
  ...
  async batchUpdateTodos(
    ...
    try {
      ...
      const result = await this.db.update(todos).set(updateValues).where(inArray(todos.id, ids));
  ...
  async getTodoStats(userId: string): Promise<TodoStats> {
    try {
      ...
      const categoryCounts = await this.db
        ...
        .from(todos)
        .where(and(eq(todos.userId, userId), isNotNull(todos.category)))
      ...
      // Get due today count
      const dueTodayResult = await this.db
        .select({ count: sql<number>`count(*)` })
        .from(todos)
        .where(
          and(
      ...
      const dueThisWeekResult = await this.db
        .select({ count: sql<number>`count(*)` })
        .from(todos)
        .where(
          and(
