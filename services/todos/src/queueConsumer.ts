 ...
 */
export async function processTodoQueue(batch: MessageBatch<TodoQueueItem>, env: Env): Promise<void> {
  ...
  const totalJobsProcessed = results
    ...
    .reduce((count, result) => count + ((result as PromiseFulfilledResult<unknown[]>).value?.length || 0), 0);
