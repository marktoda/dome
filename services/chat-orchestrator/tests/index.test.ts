...
describe('ChatOrchestrator', () => {
  ...
  describe('admin operations', () => {
    ...
    it('should delegate recordConsent to the admin controller', async () => {
      ...
      // Act
      const result = await orchestrator.recordConsent('test-user', 'chatHistory', { durationDays: 30 });
      
      // Assert
      expect(result).toEqual({ success: true });
      expect(mockControllers.admin.recordConsent).toHaveBeenCalledWith('test-user', 'chatHistory', { durationDays: 30 });
    });
  });
});