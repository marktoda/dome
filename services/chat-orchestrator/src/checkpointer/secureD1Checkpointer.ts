...
export class SecureD1Checkpointer extends BaseCheckpointSaver {
   ...
   */
  async getTuple(config: RunnableConfig): Promise<{ config: RunnableConfig; checkpoint: Checkpoint; metadata?: CheckpointMetadata; parentConfig?: RunnableConfig } | undefined> {
  ...
  async readCheckpoint(runId: string): Promise<{ checkpoint: unknown; metadata?: unknown; parentConfig?: RunnableConfig } | null> {
    try {
      ...
      if (userInfo && userInfo.role !== UserRole.ADMIN) {
        ...
        .where(and(
          ...
          eq(checkpoints.userId, userInfo.id)
        ));
      }
   ...
   */
  async *list(config: RunnableConfig, options?: { limit?: number; before?: RunnableConfig }): AsyncGenerator<{ config: RunnableConfig; checkpoint: Checkpoint; metadata?: CheckpointMetadata; parentConfig?: RunnableConfig }> {
  ...
  async writeCheckpoint(runId: string, step: string, state: unknown): Promise<void> {
    ...
    try {
      ...
      const existingRecord = await this.db.select({ userId: checkpoints.userId })
        .from(checkpoints)
        .where(eq(checkpoints.runId, runId))
        .get();
      ...
      if (existingRecord) {
        ...
        // Insert new record
        await this.db.insert(checkpoints)
          .values({
            runId: runId,
            userId: userId,
            step: step,
  ...
  async delete(runId: string): Promise<void> {
    try {
      ...
      if (userInfo && userInfo.role !== UserRole.ADMIN) {
        const record = await this.db.select({ userId: checkpoints.userId })
          .from(checkpoints)
          .where(eq(checkpoints.runId, runId))
          .get();
  ...
  async cleanup(maxAgeSeconds = this.ttlSeconds): Promise<number> {
    ...
    try {
      const result = await this.db.delete(checkpoints)
