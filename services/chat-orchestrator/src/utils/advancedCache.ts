      ...
      if (!this.revalidationPromises.has(key)) {
        ...
        const revalidationPromise = revalidate(key, entry.value)
          ...
          .catch(error => {
            this.logger.error(
              { key, error },
              'Error revalidating cache entry'
            );
            // Remove from revalidation promises
            this.revalidationPromises.delete(key);
  ...
  constructor(options: CacheOptions) {
    ...
    this.shards = Array.from({ length: this.shardCount }, (_, i) => {
      const shardOptions: CacheOptions = {
        ...
        maxMemoryUsage: options.maxMemoryUsage ? Math.ceil(options.maxMemoryUsage / this.shardCount) : undefined,
      };
  ...
  getStats(): CacheStats {
    ...
    for (let i = 0; i < shardStats.length; i++) {
      ...
      if (stats.maxMemoryUsage !== null) {
        aggregatedStats.maxMemoryUsage = (aggregatedStats.maxMemoryUsage || 0) + stats.maxMemoryUsage;
      }
    ...
    aggregatedStats.averageAccessTime = totalAccessTimeWeight > 0 
      ? weightedAccessTimeSum / totalAccessTimeWeight 
      : 0;
 ...
 */
export function getAdvancedCache<T>(name: string, options: CacheOptions): ShardedCache<T> | AdvancedCache<T> {
  if (!caches[name]) {
