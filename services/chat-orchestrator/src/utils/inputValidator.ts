...
export function validateAndSanitizeInput(
  ...
  if (content.length > opts.maxMessageLength!) {
    throw new BadRequestError(`Content exceeds maximum length of ${opts.maxMessageLength} characters`);
  }
...
export function validateInitialState(state: unknown): any {
  try {
    ...
    validatedState.messages = validateAndSanitizeMessages(
...
export function validateToolInput(toolName: string, input: unknown): unknown {
  ...
  const toolSchemas: Record<string, z.ZodTypeAny> = {
    calculator: z.object({
      expression: z.string().max(100).refine(
  ...
  for (const pattern of harmfulPatterns) {
    if (pattern.test(content)) {
      ...
      filteredContent = filteredContent.replace(
        pattern,
        '[POTENTIALLY HARMFUL CONTENT REMOVED]'
      );
    }
  }
