...
export const splitRewrite = async (state: AgentState, env: Env): Promise<AgentState> => {
  ...
  const traceId = state.metadata?.traceId || ObservabilityService.initTrace(env, state.userId, state);
  ...
  try {
    ...
    if (queryAnalysis.isComplex) {
      needsRewriting = true;
      logger.info({ reason: queryAnalysis.reason }, 'Query is complex according to analysis, needs rewriting');
    }
