...
export const generateAnswer = async (state: AgentState, env: Env): Promise<AgentState> => {
  ...
  const sourceMetadata = docs.length > 0 
    ? SearchService.extractSourceMetadata(docs)
    : [];
...
function buildSystemPrompt(
  ...
  if (formattedDocs) {
    ...
    if (includeSourceInfo) {
      prompt += 'When referencing information from these documents, include the document number in brackets, e.g., [1], to help the user identify the source.\n\n';
    }
  }
  ...
  prompt += 'Provide a helpful, accurate, and concise response based on the provided context and your knowledge. If the provided context doesn\'t contain relevant information, acknowledge this and provide the best answer you can based on general knowledge.';
