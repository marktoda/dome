...
async function determineWideningStrategy(
  ...
  const temporalPattern = /\b(recent|latest|new|old|past|history|historical|yesterday|today|last week|last month|last year)\b/i;
...
async function learnFromSuccessfulRetrievals(
  ...
  const logger = getLogger().child({ node: 'dynamicWiden', function: 'learnFromSuccessfulRetrievals' });
