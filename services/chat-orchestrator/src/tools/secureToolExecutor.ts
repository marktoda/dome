...
export class SecureToolExecutor {
      ...
      return Array.from(this.tools.values()).filter(tool => {
        ...
        if (tool.requiredPermissions.length > 0) {
          const hasAllPermissions = tool.requiredPermissions.every(
            permission => userInfo.permissions?.includes(permission) || userInfo.permissions?.includes('*')
          );
  ...
  async detectToolIntent(query: string, toolName: string): Promise<ToolIntentDetection> {
    ...
    const toolPatterns: Record<string, RegExp[]> = {
      calculator: [
        /calculate|compute|evaluate|solve|math|equation|arithmetic|formula/i,
      ],
      calendar: [
        /calendar|schedule|appointment|meeting|event|reminder|date/i,
      ],
      weather: [
        /weather|temperature|forecast|rain|snow|sunny|cloudy|humidity|wind/i,
      ],
      web_search: [
        /search|find|look up|google|information about|tell me about|what is/i,
      ],
    };
  ...
  async executeTool(
    ...
    try {
      ...
      if (tool.riskLevel >= 3 && !intentDetection.isValid) {
        this.logger.warn(
          { toolName, intentDetection },
          'Tool intent validation failed'
        );
        
        throw new ForbiddenError(
          `Tool intent validation failed: ${intentDetection.reason}`
        );
      }
  ...
  return {
    ...
    async execute(input: unknown): Promise<unknown> {
      ...
      try {
        ...
        throw new BadRequestError(`Failed to evaluate expression: ${error instanceof Error ? error.message : String(error)}`);
      }
    },
  };
}
