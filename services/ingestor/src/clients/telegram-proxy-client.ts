...
export class TelegramProxyClient {
  ...
  async getMessageHistory(
    ...
    if (offsetId) {
      queryParams.append('offsetId', offsetId.toString());
    }
   ...
   */
  async sendMessage(sessionId: string, chatId: string, message: string): Promise<any> {
    // Build URL
    ...
    for (let attempt = 0; attempt < this.config.maxRetries!; attempt++) {
      try {
        ...
        if (attempt < this.config.maxRetries! - 1) {
          ...
          const delay =
            this.config.retryDelay! * Math.pow(2, attempt) * (0.5 + Math.random() * 0.5);
