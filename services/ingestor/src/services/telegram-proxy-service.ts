...
import { TelegramProxyClient, MessagePaginationOptions, MessagePollingOptions } from '../clients/telegram-proxy-client';
...
export class TelegramProxyService implements ITelegramService {
  ...
  async collectMessages(
    ...
    try {
      ...
      const response = await this.config.proxyClient.getMessages(
        sessionId,
        source,
        options
      );
  ...
  async collectMedia(
    ...
    try {
      ...
      const response = await this.config.proxyClient.getMessageHistory(
        sessionId,
        source,
        {
          limit: options.limit,
      ...
      const mediaMessages = response.messages.filter(msg => {
        ...
        const hasMedia = msg.media && (
          ...
          (options.mediaType === 'video' && msg.media.document && msg.media.document.mimeType?.startsWith('video/')) ||
  ...
  async getSourceInfo(userId: number, source: string): Promise<any> {
    ...
    try {
      ...
      const chatInfo = response.chats.find((chat: any) => 
        chat.id.toString() === source || 
        chat.username === source
      );
  ...
  async pollMessages(
    ...
    try {
      ...
      const response = await this.config.proxyClient.pollMessages(
        sessionId,
        source,
        options
      );
   ...
   */
  private async getSessionWithRetry(userId: number): Promise<{ session: TelegramSession; sessionId: string }> {
    // Check cache first
   ...
   */
  private async refreshSession(userId: number): Promise<{ session: TelegramSession; sessionId: string }> {
    // Remove from cache
