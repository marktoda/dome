...
import { VectorizeService, DEFAULT_VECTORIZE_CONFIG, createVectorizeService } from '../../src/services/vectorize';
...
describe('VectorizeService', () => {
  ...
  describe('upsert', () => {
    ...
    it('should split large batches into smaller ones', async () => {
      ...
      const vectors: VectorWithMetadata[] = Array(5).fill(0).map((_, i) => ({
        id: `note:note${i}`,
        values: [0.1, 0.2, 0.3],
        metadata: {
          userId: 'user1',
  ...
  describe('createVectorizeService', () => {
    ...
    it('should create a vectorize service with custom config when provided', () => {
      const customConfig = {
        maxBatchSize: 50,
        retryAttempts: 5
      };
      
      const vectorizeService = createVectorizeService(mockVectorize, customConfig);
      
      // We can't directly access private properties, so we'll test indirectly
      // by checking that the custom batch size is used
      
      // Create a batch of 60 vectors
      const vectors: VectorWithMetadata[] = Array(60).fill(0).map((_, i) => ({
        id: `note:note${i}`,
        values: [0.1, 0.2, 0.3],
        metadata: {
          userId: 'user1',
      ...
      vectorizeService.upsert(vectors);
      
      // With maxBatchSize of 50, we should get 2 batches
      expect(mockVectorize.upsert).toHaveBeenCalledTimes(2);
    });
  });
});
