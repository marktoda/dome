...
import { Env, QueueMessage, VectorWithMetadata, CFExecutionContext } from '../src/types';
  ...
  describe('RPC Methods', () => {
    describe('embed', () => {
      ...
      it('should handle errors', async () => {
        ...
        await expect((Constellation as any).embed(mockEnv, job)).rejects.toThrow('AI service error');
      });
    });
    ...
    describe('query', () => {
      ...
      it('should handle errors', async () => {
        ...
        await expect((Constellation as any).query(mockEnv, 'test', { userId: 'user1' })).rejects.toThrow('Vectorize service error');
      });
    });
    ...
    describe('stats', () => {
      ...
      it('should handle errors', async () => {
        // Mock Vectorize service to throw an error
        mockEnv.VECTORIZE.describe = vi.fn().mockRejectedValue(new Error('Vectorize service error'));
        
        await expect((Constellation as any).stats(mockEnv)).rejects.toThrow('Vectorize service error');
      });
    });
  });
