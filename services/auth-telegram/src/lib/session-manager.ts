 ...
 */
import type { TelegramSession, TelegramSessionDTO } from '../models/session';
import { fromDTO as sessionFromDTO, toDTO as sessionToDTO } from '../models/session';
...
export class SessionManager {
  ...
  async saveSession(
    ...
    const { encryptedData, iv } = await encrypt(sessionString, this.sessionSecret, salt);
  ...
  async getSessionByUserId(
    userId: number,
  ): Promise<{ sessionString: string; sessionId: string; expiresAt: Date }> {
    // Get from database
    const result = await this.db
      .prepare(
        `
  ...
  private async decryptSession(session: TelegramSession): Promise<string> {
    ...
    return decrypt(session.encryptedData, session.iv, this.sessionSecret, salt);
  }
}
