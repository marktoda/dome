...
export function createJwtAuthMiddleware(options: JwtOptions): MiddlewareHandler {
  const { secretKey, issuer, audience, headerName = 'authorization' } = options;
...
export function createApiKeyAuthMiddleware(options: ApiKeyOptions): MiddlewareHandler {
  const { headerName = 'x-api-key', envVarName = 'API_KEY' } = options;
...
export function createUserRateLimitMiddleware(options: RateLimitOptions = {}): MiddlewareHandler {
  const { windowMs = 60000, maxRequests = 100, userBased = true } = options;
  ...
  const rateLimitMiddleware = createRateLimitMiddleware(windowMs, maxRequests, keyGenerator);
