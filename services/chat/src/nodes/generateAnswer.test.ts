...
describe('generateAnswer Node', () => {
  ...
  beforeEach(() => {
    ...
    mockState = {
      userId: 'user-123',
      messages: [{ role: 'user', content: 'What do you know about quantum computing?' }],
      options: {
  ...
  it('should transform state with RAG-enabled answer generation', async () => {
    ...
    expect(ModelFactory.createChatModel).toHaveBeenCalledWith(mockEnv, {
      modelId: 'gpt-4-turbo',
      temperature: 0.3,
    ...
    expect(reduceRagContext).toHaveBeenCalledWith(mockState, expect.any(Number));
