  ...
  try {
    ...
    if (suggestedAction === 'proceed' && isAdequate && overallScore > 0.7) {
      ...
      const classification: ToolNecessityClassification = {
        isToolNeeded: false,
        reasoning:
          'Retrieved information is highly relevant and adequate. No external tools needed.',
        confidence: 0.9,
      };
...
REASONING: [Your explanation]`;
    ...
    const decisionMatch = classificationText.match(
      /DECISION:\s*(Tools are needed|Tools are not needed)/i,
    );
    ...
    const confidence = confidenceMatch ? parseFloat(confidenceMatch[1]) : isToolNeeded ? 0.8 : 0.7;

    const classificationReasoning = reasoningMatch ? reasoningMatch[1].trim() : classificationText;
