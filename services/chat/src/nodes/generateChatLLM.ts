...
export async function generateChatLLM(
  ...
  try {
    ...
    if (cfg.configurable?.stream) {
      ...
      // Start streaming
      const stream = await streamingModel.stream(chatMessages.map(m => ({
        role: m.role,
        content: m.content,
      })));
      ...
      const response = await streamingModel.invoke(chatMessages.map(m => ({
        role: m.role,
        content: m.content,
      })));
    ...
    fullText = "I apologize, but I encountered an error while generating your answer. Please try again.";
  }
