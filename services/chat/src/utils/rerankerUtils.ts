...
export interface RerankerOptions {
   ...
   * For Workers AI: '@cf/baai/bge-reranker-base', etc.
   */
  model: string;
  ...
  /** Which reranker implementation to use */
  implementation?: 'cohere' | 'workers-ai';
  /** Cohere API key if using Cohere reranker */
  cohereApiKey?: string;
}
...
/**
 * Creates a reranker function that applies reranking to retrieval results
 * Can use either Cohere's reranker or Workers AI reranker based on the implementation option
 *
 * @param options Configuration options for the reranker
...
export function createReranker(options: RerankerOptions) {
  const {
    ...
    implementation = 'cohere', // Default to Cohere implementation
    cohereApiKey,
  } = options;

  const logger = getLogger().child({ component: name });

  // Use Cohere reranker if specified
  ...
  // Otherwise use Workers AI reranker
  return createWorkersAIReranker({
    name,
    model,
    scoreThreshold,
    maxResults,
...
/**
 * Creates a reranker function that uses Cohere's reranker
 *
 * @param options Configuration options for the reranker
 * @returns A function that reranks retrieval results using Cohere
 */
function createCohereReranker(options: RerankerOptions) {
  const {
    name,
  ...
  const logger = getLogger().child({ component: `${name}-cohere` });
  ...
  return async function rerank(
    ...
    const rerankerSpanId = ObservabilityService.startSpan(
      env,
      traceId,
      `rerank_${name}`,
      minimalState,
    ...
    try {
      ...
      // We're in the Cohere implementation - use Cohere reranker
      const rerankedChunks = await rerankWithCohere(
      ...
      if (!keepBelowThreshold) {
        filteredChunks = rerankedChunks.filter(chunk => {
          ...
          const sourceThreshold =
            SOURCE_THRESHOLDS[contentType as keyof typeof SOURCE_THRESHOLDS] || scoreThreshold;
...
/**
 * Helper function to cleanly handle fallback to vector scores
 *
 * @param chunks The original chunks to use for fallback
 * @returns Chunks with fallback scores based on vector similarity
 */
function fallbackToVectorScores(chunks: DocumentChunk[]): DocumentChunk[] {
  return chunks
    .map(chunk => {
      const contentType = (chunk.metadata.sourceType as string) || 'note';
      const sourceThreshold =
        SOURCE_THRESHOLDS[contentType as keyof typeof SOURCE_THRESHOLDS] || 0.45;
      const vectorScore = chunk.metadata.relevanceScore || 0.5;

      // Create a new chunk with fallback score information
    ...
    .sort(
      (a, b) => ((b.metadata as any).hybridScore || 0) - ((a.metadata as any).hybridScore || 0),
    );
}

/**
 * Creates a reranker function that uses Workers AI
 *
 * @param options Configuration options for the reranker
 * @returns A function that reranks retrieval results using Workers AI
 */
function createWorkersAIReranker(options: RerankerOptions) {
  const {
    name,
    model,
    scoreThreshold = 0.45,
  ...
  const logger = getLogger().child({ component: `${name}-workers-ai` });
  ...
  return async function rerank(
    ...
    try {
      ...
      // Use Workers AI for reranking
      const rerankedChunks = await rerankWithWorkersAI(retrievalResult.chunks, query, model, env);

      // Rest of the function is the same as in createCohereReranker...
      // Continue with filtering, creating the result, etc.
      ...
      // Error handling like in createCohereReranker
      const domeError = toDomeError(error);
      logger.error({ err: domeError }, `Error during ${name} reranking`);

      const executionTimeMs = performance.now() - startTime;

      // Return fallback result
      const fallbackChunks = fallbackToVectorScores(retrievalResult.chunks);

      // Create and return fallback result
      let fallbackCategory: RetrievalToolType;
      switch (retrievalResult.sourceType) {
        case 'code':
          fallbackCategory = RetrievalToolType.CODE;
      ...
      const fallbackResult: RetrievalTask = {
        category: fallbackCategory,
        query,
        chunks: fallbackChunks.slice(0, maxResults),
        sourceType: retrievalResult.sourceType,
        metadata: {
          rerankerModel: model,
          executionTimeMs,
...
// Move these helper functions and interfaces to the top level
...
async function rerankWithCohere(
  ...
  const logger = getLogger().child({ component: 'CohereReranker' });
  ...
  try {
    ...
    const apiKey = cohereApiKey || (env as any)?.COHERE_API_KEY;

    if (!apiKey) {
      logger.warn('No Cohere API key found - falling back to vector scores');
      return fallbackToVectorScores(chunks);
    }
    ...
    const cohereRerank = new CohereRerank({
      apiKey,
      model: modelName || 'rerank-english-v2.0',
    });

    // Perform the reranking
    const rerankedResults = await cohereRerank.rerank(docs, query);

    logger.info(
      {
        rerankedResults: rerankedResults.slice(0, 3),
      },
      'Cohere reranking results (top 3)',
    );

    // Map the reranked results back to our chunks
    const rerankedChunks = rerankedResults
      .map(result => {
        const chunk = chunks[result.index];

        // Get source type for threshold lookup
        const contentType = chunk.metadata.sourceType || 'note';
        // Get source-specific threshold
        const sourceThreshold =
          SOURCE_THRESHOLDS[contentType as keyof typeof SOURCE_THRESHOLDS] || 0.45;

        // Add the reranker score to the chunk
        return {
          ...chunk,
          metadata: {
            ...chunk.metadata,
            rerankerScore: result.relevanceScore,
            // Combine with the original vector score (30% weight to vector score)
            hybridScore: 0.7 * result.relevanceScore + 0.3 * (chunk.metadata.relevanceScore || 0.5),
            sourceThreshold,
          },
        };
      })
    ...
    logger.info(
      {
        ...
        ranked: rerankedChunks.slice(0, 5).map(c => ({
          id: c.id,
          sourceType: c.metadata.sourceType,
          rerankerScore: (c.metadata as any).rerankerScore,
          hybridScore: (c.metadata as any).hybridScore,
...
/**
 * Reranks documents using Workers AI
 */
async function rerankWithWorkersAI(
  ...
  try {
    ...
    const rerankerOutput = (await envWithAI.AI.run(
      effectiveModel,
      rerankerInput,
    )) as RerankerResponse;
    ...
    const rerankedChunks = chunks.map((chunk, i) => {
      ...
      // Use type assertion to handle the additional properties we're adding to metadata
      const enhancedMetadata = {
        ...chunk.metadata,
    ...
    logger.info(
      {
        ...
        ranked: rerankedChunks.map(c => ({
          id: c.id,
          sourceType: c.metadata.sourceType,
          rawScore: (c.metadata as any).rerankerRawScore,
          normalizedScore: (c.metadata as any).rerankerScore,
          hybridScore: (c.metadata as any).hybridScore,
          vectorScore: c.metadata.relevanceScore,
        })),
      },
