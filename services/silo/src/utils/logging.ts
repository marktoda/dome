...
export const siloMetrics = createServiceMetrics('silo');

/**
 * Get a logger instance with the silo service name
 */
export function getLogger(): any {
  return getDomeLogger().child({ service: 'silo' });
}
...
export function logError(error: unknown, message: string, context: Record<string, any> = {}): void {
  ...
  siloMetrics.trackOperation('error', false, {
...
export async function trackOperation<T>(
  ...
  return domeTrackOperation(operationName, fn, { service: 'silo', ...context });
}
