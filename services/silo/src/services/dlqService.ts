...
export class DLQServiceImpl implements DLQService {
  ...
  async storeDLQMessage<T>(message: DLQMessage<T>): Promise<string> {
    return trackOperation('silo.dlq.storeDLQMessage', async () => {
      try {
        ...
        getLogger().info(
          logContext,
          `Successfully stored DLQ message with ID ${id}`
        );
        ...
        const domeError = toDomeError(
          error,
          'Failed to store DLQ message metadata',
          {
            operation: 'storeDLQMessage',
            originalMessageId: message?.processingMetadata?.messageId,
            queueName: message?.processingMetadata?.queueName,
  ...
  async getDLQStats(): Promise<DLQStats> {
    return trackOperation('silo.dlq.getStats', async () => {
      try {
        ...
        // Get total count
        getLogger().debug({ operation: 'getDLQStats', step: 'getTotalCount' }, 'Getting total message count');
        const totalResult = await this.db.select({ count: count() }).from(dlqMetadata).get();
        const total = Number(totalResult?.count || 0);

        // Get reprocessed count
        getLogger().debug({ operation: 'getDLQStats', step: 'getReprocessedCount' }, 'Getting reprocessed message count');
        ...
        getLogger().debug({ operation: 'getDLQStats', step: 'getQueueCounts' }, 'Getting counts by queue name');
        ...
        getLogger().debug({ operation: 'getDLQStats', step: 'getErrorCounts' }, 'Getting counts by error type');
        ...
        }, `Retrieved DLQ statistics in ${duration.toFixed(2)}ms: ${total} total, ${total - reprocessed} pending, ${reprocessed} reprocessed`);
  ...
  async markAsReprocessed(id: string, result: string): Promise<void> {
    return trackOperation('silo.dlq.markAsReprocessed', async () => {
      try {
        // Validate inputs
        ...
        getLogger().info(
          logContext,
          `Successfully marked DLQ message ${id} as reprocessed`
        );
        ...
        const domeError = toDomeError(
          error,
          `Failed to mark DLQ message ${id} as reprocessed`,
          { messageId: id, operation: 'markAsReprocessed' }
        );
  ...
  async reprocessMessage(id: string): Promise<string> {
    return trackOperation('silo.dlq.reprocessMessage', async () => {
      try {
        // Validate inputs
        ...
        getLogger().info(
          logContext,
          `Starting reprocessing of DLQ message ${id}`
        );
        ...
        if (queueName === 'silo-ingest-queue') {
          try {
            ...
            if (validationError instanceof z.ZodError) {
              ...
              metrics.increment('silo.dlq.reprocessing_errors', 1, { error_type: 'validation_error' });
        ...
        const domeError = toDomeError(
          ...
          { messageId: id, operation: 'reprocessMessage' }
        );
  ...
  async reprocessMessages(ids: string[]): Promise<Record<string, string>> {
    return trackOperation('silo.dlq.reprocessMessages', async () => {
      // Validate input
      ...
      getLogger().info(
        logContext,
        `Starting batch reprocessing of ${ids.length} DLQ messages`
      );
  ...
  async purgeMessages(options: DLQFilterOptions = {}): Promise<number> {
    return trackOperation('silo.dlq.purgeMessages', async () => {
      try {
  ...
  async sendToDLQ<T>(
    ...
    return trackOperation('silo.dlq.sendToDLQ', async () => {
      try {
        // Validate parameters
        ...
        if (this.env.INGEST_DLQ) {
          try {
            ...
            getLogger().debug(
              logContext,
              `Successfully sent message to INGEST_DLQ queue`
            );
          } catch (queueError) {
        ...
        getLogger().info(
          ...
          `Successfully processed DLQ message - DB ID: ${id}, queue send: ${queueSendResult ? 'success' : 'skipped'}`
        );
