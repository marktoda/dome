  ...
  program
    ...
    .action(async (context: string, options: { content?: string }) => {
      ...
      try {
        if (options.content) {
          ...
          rl.on('line', async (line) => {
            ...
            try {
              ...
              console.log(error(`Failed to add line: ${err instanceof Error ? err.message : String(err)}`));
            }
        ...
        console.log(error(`Failed to add note: ${err instanceof Error ? err.message : String(err)}`));
        process.exit(1);
      }
    });
}