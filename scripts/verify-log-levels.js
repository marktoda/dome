    ...
    if (opData.pattern.test(content)) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (opData.pattern.test(line)) {
          ...
          serviceData.operations[opName].levels[usedLevel] =
            (serviceData.operations[opName].levels[usedLevel] || 0) + 1;
...
function processService(serviceName, serviceDir) {
  ...
  const totalLogStatements = LOG_LEVELS.reduce(
    (sum, level) => sum + serviceData.logLevelCounts[level],
    0,
  );
...
function generateRecommendations() {
  ...
  if (errorPercentage < 5) {
    summary.recommendations.push(
      'Consider increasing error logging for critical issues (recommended: 5-10%)',
    );
  }

  if (warnPercentage < 10) {
    summary.recommendations.push(
      'Consider increasing warning logging for potential issues (recommended: 10-15%)',
    );
  }

  if (infoPercentage < 50) {
    summary.recommendations.push(
      'Consider increasing info logging for operational visibility (recommended: 50-70%)',
    );
  }

  if (debugPercentage > 30) {
    summary.recommendations.push(
      'Consider reducing debug logging in production code (recommended: 10-15%)',
    );
  }
  ...
  for (const serviceName in results.services) {
    ...
    if (service.issues.length > 0) {
      summary.recommendations.push(
        `Service ${serviceName} needs attention: ${service.issues.join(', ')}`,
      );
    }
  }
}
...
function generateReport() {
  ...
  for (const [opName, opData] of Object.entries(results.summary.operationConsistency)) {
    ...
    const consistencyStatus =
      correctPercentage >= 90 ? '✅ High' : correctPercentage >= 70 ? '🟨 Medium' : '❌ Low';
  ...
  for (const serviceName in results.services) {
    ...
    const totalServiceLogs = LOG_LEVELS.reduce(
      (sum, level) => sum + service.logLevelCounts[level],
      0,
    );
...
async function main() {
  ...
  const services = fs.readdirSync(SERVICES_DIR).filter(item => {
