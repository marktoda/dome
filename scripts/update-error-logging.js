...
const findTsFiles = () => {
  try {
    const output = execSync('find services packages -type f -name "*.ts" | grep -v "node_modules" | grep -v "dist"').toString();
    return output.split('\n').filter(Boolean);
  } catch (error) {
...
const processFile = (filePath) => {
  try {
    ...
    for (const errorVar of errorVars) {
      const errorVarRegex = new RegExp(`(\\s+)(logger|getLogger\\(\\))\\.error\\(\\s*\\{\\s*${errorVar}(\\s*,|\\s*\\})`, 'g');
      ...
      if (errorVarRegex.test(content)) {
        ...
        const errorMessageRegex = new RegExp(`const\\s+errorMessage\\s*=\\s*${errorVar}\\s+instanceof\\s+Error`);
        if (!errorMessageRegex.test(content)) {
          ...
          while ((match = errorVarRegex.exec(content)) !== null) {
            ...
            if (!prevLine.includes(`const errorMessage = ${errorVar} instanceof Error`)) {
              ...
              content = content.slice(0, lineStart) + errorMessageExtraction + content.slice(lineStart);
              
              // Update position after insertion
              const insertedLength = errorMessageExtraction.length;
              position += insertedLength;
              lineStart += insertedLength;
              
              // Update the logger.error call to include errorMessage
              const loggerErrorCall = content.substring(position, content.indexOf('\n', position));
              if (!loggerErrorCall.includes('errorMessage')) {
                const updatedCall = loggerErrorCall.replace(/(\{\s*error)(\s*,|\s*\})/, '$1, errorMessage$2');
                content = content.slice(0, position) + updatedCall + content.slice(position + loggerErrorCall.length);
              }
