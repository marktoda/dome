...
function processService(serviceName, serviceDir) {
  ...
  sourceFiles.forEach(filePath => {
    ...
    if (
      !results.services[serviceName].usingDomeLogging &&
      checkPattern(filePath, PATTERNS.DOME_LOGGING_IMPORT)
    ) {
      results.services[serviceName].usingDomeLogging = true;
    }

    if (
      !results.services[serviceName].usingDomeErrors &&
      checkPattern(filePath, PATTERNS.DOME_ERRORS_IMPORT)
    ) {
    ...
    if (
      checkPattern(filePath, PATTERNS.CONSOLE_ERROR) ||
    ...
    if (
      checkPattern(filePath, PATTERNS.THROW_STRING) ||
      checkPattern(filePath, PATTERNS.THROW_NEW_ERROR)
    ) {
...
function runRipgrep(pattern) {
  try {
    return execSync(`cd ${SERVICES_DIR} && rg -l "${pattern}" --type ts --type js`, {
      encoding: 'utf8',
    })
      .trim()
      .split('\n')
