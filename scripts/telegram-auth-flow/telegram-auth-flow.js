...
async function sendAuthCode(phoneNumber) {
  ...
  try {
    ...
    if (!data.success) {
      ...
      if (
        data.error?.message?.includes('WebSocket connection failed') ||
...
async function main() {
  try {
    ...
    const verifyResult = await verifyAuthCode(PHONE_NUMBER, sendCodeResult.phoneCodeHash, code);
    ...
    if (
      error.message.includes('API ID or Hash cannot be empty') ||
    ...
    if (
      ...
      console.log(
        '2. Run the Telegram client on a dedicated server instead of a Cloudflare Worker',
      );
      console.log(
        '3. Use a different library that is compatible with the Cloudflare Workers environment',
      );
    }
  } finally {
    rl.close();
