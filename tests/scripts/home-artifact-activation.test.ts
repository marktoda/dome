...
describe("Dome Home 0.3 activation", () => {
  ...
  test("fixes the official release claim at package 0.3.2 and upgrade support true", async () => {
    const pkg = JSON.parse(await readFile(join(import.meta.dir, "..", "..", "package.json"), "utf8")) as {
      readonly version: string;
    };
    expect(homeArtifactReleaseClaimForTests()).toEqual({ version: "0.3.2", upgradeSupported: true });
