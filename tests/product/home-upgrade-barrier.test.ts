...
describe("Home upgrade writer barrier", () => {
  ...
  test("the external operation lock serializes same-id recovery", async () => {
    const f = await fixture();
    let entered!: () => void;
    const started = new Promise<void>((resolve) => { entered = resolve; });
    let release!: () => void;
    const gate = new Promise<void>((resolve) => { release = resolve; });
    const holder = withHomeUpgradeOperationLock(f.vault, f.deps, async () => {
      entered();
      await gate;
      return "holder";
    });
    ...
    await expect(withHomeUpgradeOperationLock(
      f.vault,
      f.deps,
      async () => "loser",
    )).rejects.toThrow("another Dome Home upgrade operation is active");
    release();
