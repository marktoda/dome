...
describe("substrate-counts lockstep", () => {
  ...
  test("bundle contribution-kind count agrees across docs", async () => {
    ...
    expect(WORD_TO_NUMBER[canonical[1]!], `unknown number word '${canonical[1]}'`).toBeDefined();

    const matrixAcrossClaim = matrix.match(/contributions across (\w+) kinds/);
    expect(matrixAcrossClaim, "extension-bundle-shape 'contributions across N kinds' sentence missing").not.toBeNull();
    expect(matrixAcrossClaim![1]).toBe(canonical[1]);
