...
export class HarnessImpl implements Harness {
  ...
  async cleanup(): Promise<void> {
    await this.activity.track(
      ...
      async (advance) => {
        ...
        advance("remove temporary vault");
        try {
  ...
  async userCommit(input: UserCommitInput): Promise<CommitOid> {
    return this.activity.track("userCommit", "snapshot vault", async (advance) => {
      await this.snapshot();
      advance("write owner files");
      ...
      advance("check always-true invariants");
  ...
  async tick(): Promise<TickResult> {
    return this.activity.track("tick", "snapshot vault", async (advance) => {
      await this.snapshot();
      advance("detect Git drift");
  ...
  async runCli(args: ReadonlyArray<string>): Promise<{
    ...
    return this.activity.track("runCli", "snapshot vault", async (advance) => {
      await this.snapshot();
      ...
      try {
        ...
        advance("reopen harness runtime");
        this.runtime = await openRuntime(this.vaultPath, this.modelProvider);
      }
