...
async function validateD1Databases() {
  ...
  try {
    ...
    const d1Changes = previewData.steps.filter((step: any) => 
      step.resource && step.resource.type.includes('D1Database')
    );
...
async function validateR2Buckets() {
  ...
  try {
    ...
    const r2Changes = previewData.steps.filter((step: any) => 
      step.resource && step.resource.type.includes('R2Bucket')
    );
