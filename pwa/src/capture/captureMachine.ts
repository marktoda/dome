export type CapturePhase = "idle" | "recording" | "transcribing" | "review" | "saving";
...
export function captureReducer(s: CaptureState, a: CaptureAction): CaptureState {
  switch (a.kind) {
    ...
    case "save": return s.phase === "review" && s.draft.trim().length > 0 ? { ...s, phase: "saving" } : s;
    case "saved": return s.phase === "saving" ? INITIAL : s;
