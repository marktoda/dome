...
const aiSchema = z.object({
  // Model Selection
  DOME_DEFAULT_MODEL: z.string().default('gpt-4o-mini').describe('Default AI model for generation'),
  DOME_EMBEDDING_MODEL: z.string().default('text-embedding-3-small').describe('Model for embeddings'),
  DOME_SUMMARIZER_MODEL: z.string().default('gpt-4o-mini').describe('Model for summarization'),
  
  // Model Parameters
  DOME_AI_TEMPERATURE: z.coerce.number().min(0).max(2).default(0.3).describe('AI temperature setting'),
  DOME_AI_MAX_TOKENS: z.coerce.number().default(500).describe('Maximum tokens for AI responses'),
});
...
export const config = {
  ...
  ai: {
    models: {
      default: rawConfig.DOME_DEFAULT_MODEL,
      embedding: rawConfig.DOME_EMBEDDING_MODEL,
      summarizer: rawConfig.DOME_SUMMARIZER_MODEL,
    },
    temperature: rawConfig.DOME_AI_TEMPERATURE,
    maxTokens: rawConfig.DOME_AI_MAX_TOKENS,
  },
...
export function validateConfig(): void {
  ...
  if (errors.length > 0) {
    ...
    if (config.isProduction) {
      ...
      logger.warn('Configuration validation warnings detected (non-fatal in development)');
    }
  }
}
...
export function getAIModel(task: 'default' | 'embedding' | 'summarizer' = 'default'): string {
  return config.ai.models[task] || config.DOME_DEFAULT_MODEL || 'gpt-4o-mini';
}
