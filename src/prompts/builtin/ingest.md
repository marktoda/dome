...
6. Append a `log.md` entry summarizing what you did via `appendLog`.
7. When done processing, call `deleteDocument` on the original inbox file. The wiki/source pages you created are the durable record; the inbox file's job is complete (INBOX_IS_EPHEMERAL).
