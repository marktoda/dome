...
/**
 * Create a context-aware notes agent
 */
export async function createContextAwareAgent(currentPath?: string): Promise<Agent> {
  let instructions = baseInstructions;
  
  // If a current path is provided, check for context-specific instructions
  if (currentPath) {
    const manager = new ContextManager(vaultPath);
    const contextResult = await manager.findContextForPath(currentPath);
    
    if (contextResult?.context.aiInstructions) {
      instructions += `\n\n## Context-Specific Instructions\n\nYou are currently working in the "${contextResult.context.name}" context.\n${contextResult.context.aiInstructions}`;
    }
  }
  
  return new Agent({
    name: 'Context-Aware Notes Agent',
    instructions,
...
/**
 * Get context information for a path
 */
export async function getContextInfo(path: string): Promise<{ name: string; instructions?: string } | null> {
  const manager = new ContextManager(vaultPath);
  const contextResult = await manager.findContextForPath(path);
  
  if (!contextResult) return null;
  
  return {
    name: contextResult.context.name,
    instructions: contextResult.context.aiInstructions,
  };
}