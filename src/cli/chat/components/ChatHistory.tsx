...
const MessageRow: React.FC<{ 
  ...
  const timestamp = (() => {
    ...
    if (timestampMode === 'absolute') {
      return message.timestamp.toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' });
    }
    // relative
  ...
  const getColor = (type: ChatMessage['type']) => {
    switch (type) {
      case 'user': return COLORS.you;
      case 'assistant': return COLORS.dome;
  ...
  return (
    ...
    >
      <Box marginBottom={message.type === 'assistant' ? 1 : 0}>
        <Text color={getColor(message.type)} bold>{getPrefix(message.type)}</Text>
...
export const ChatHistory = React.memo<ChatHistoryProps>(({ messages, timestampMode, selectedMessageIndex }) => {
  ...
  return (
    <Box 
      ...
      overflow="hidden"
    >
      <Static items={staticMessages}>{(msg: ChatMessage, idx: number) => (
        <MessageRow 
          key={msg.id}
          message={msg}
